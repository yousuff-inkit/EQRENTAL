...
function setCollection(){
	...
	function funReadOnly(){
		 setCollection();
		$('#frmRentalCloseCosmo input').attr('readonly',true);
		$('#frmRentalCloseCosmo select').attr('disabled',true);
...
/* 		getCheckin();
	...
	function funNotify(){
		...
		var intimepriorstatus=checkPriorTime(intimeprior);
...
if(document.getElementById("rentalagent").value==""){
	...
	function funRemoveReadOnly(){
		$('#frmRentalCloseCosmo input').attr('readonly',false);
		$('#frmRentalCloseCosmo select').attr('disabled',false);
...
if(document.getElementById("rentalagent").value==""){
	...
	function setValues(){
		...
		if($('#voucherno').val()!='' && $('#msg').val()=='Successfully Saved'){
			$.messager.confirm('Confirm', 'Do you want to Generate Reciepts?', function(r){
	 			if (r){
					var url=document.URL;
					var reurl=url.split("com/");
					top.addTab("Rental Receipts",reurl[0]+"com/operations/commtransactions/rentalreceipts/rentalReceipts.jsp?mode=A&cldocno="+document.getElementById("clientid").value+"&createmode=1");
				}
			});
		}
}
...
<form id="frmRentalCloseCosmo" action="saveRentalCloseCosmo" autocomplete="off">
	<%-- <script>
