...
function closing(){
	...
	if (($("#mode").val() == "view") && $("#agreementno").val()!="") {
        ...
        var reurl=url.split("saveRentalCloseCarFare");
        //$("#docno").prop("disabled", false);                

        var win= window.open(reurl[0]+"printRAClosingSummaryCarfare?docno="+document.getElementById("agreementno").value,"_blank","top=85,left=150,Width=1020,Height=600,location=no,scrollbars=no,toolbar=yes");
 	   win.focus();
 	   } 
...
function setCollection(){
	...
	function funReadOnly(){
		 setCollection();
		$('#frmRentalCloseCarFare input').attr('readonly',true);
		$('#frmRentalCloseCarFare select').attr('disabled',true);
...
if(document.getElementById("rentalagent").value==""){
	...
	function funRemoveReadOnly(){
		$('#frmRentalCloseCarFare input').attr('readonly',false);
		$('#frmRentalCloseCarFare select').attr('disabled',false);
...
<form id="frmRentalCloseCarFare" action="saveRentalCloseCarFare" autocomplete="off">
	<%-- <script>
