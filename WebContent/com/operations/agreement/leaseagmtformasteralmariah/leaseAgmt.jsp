...
function closing(){
	 if (($("#mode").val() == "view") && $("#masterdoc_no").val()!="") {
	      var url=document.URL;
	        var reurl=url.split("saveLeaseAgreementForMasterAlmariah");
	        $("#docno").prop("disabled", false);                

	        var win= window.open(reurl[0]+"printLAGClosingSummaryAlmariah?docno="+document.getElementById("masterdoc_no").value,"_blank","top=85,left=150,Width=1020,Height=600,location=no,scrollbars=no,toolbar=yes");
	      win.focus();
	     } 
	  ...
	  // tariff grid
			 ...
			 var rows = $("#rateGrid").jqxGrid('getrows');
			   ...
			   newTextBox.val(rows[i].rate+" :: "+rows[i].cdw+" :: "
					   ...
					   +rows[i].exkmrte+" :: "+rows[i].chaufchg+" :: "+rows[i].chaufexchg+" :: "+rows[i].status+" :: "+rows[i].totalcostpermonth+" :: "+rows[i].drivercostpermonth+" :: "+rows[i].securitypass+" :: "+rows[i].fuel+" :: "+rows[i].salik+" :: "+rows[i].safetyacc+" :: "+rows[i].diw+" :: "+rows[i].hpd+" :: "+rows[i].rateperexhr+" :: ");
...
// var win= window.open(reurl[0]+"/epicraprint1.jsp","_blank","top=250,left=310,Width=800,Height=800,location=no,scrollbars=no,toolbar=yes");
    var win= window.open(reurl[0]+"LeasemainprintAlmariah?docno="+document.getElementById("masterdoc_no").value+"&formdetailcode=LAG","_blank","top=250,left=310,Width=800,Height=800,location=no,scrollbars=no,toolbar=yes");
    win.focus(); 
	   } 
...
var win= window.open(reurl[0]+"LeasemasterPrintAlmariah?docno="+document.getElementById("masterdoc_no").value+"&formdetailcode=LAG","_blank","top=250,left=310,Width=800,Height=800,location=no,scrollbars=no,toolbar=yes");
win.focus(); 
	   } 
...
<form id="frmLeaseAgreementForMaster" action="saveLeaseAgreementForMasterAlmariah" method="post" autocomplete="off">
...
<input type="hidden" name="advchkval" id="advchkval" value='<s:property value="advchkval"/>'  /> 
       ...
       <input type="hidden" id="hidmasterrefsrno" name="hidmasterrefsrno" value='<s:property value="hidmasterrefsrno"/>' />
</div>
