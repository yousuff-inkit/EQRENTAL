	...
	for(int k=0;k<psrnoarray.size();k++)
	{
		...
		String[] pmgntarr=((String) psrnoarray.get(k)).split("::"); 
		 ...
		 if(Integer.parseInt(naliasdocno)>0)
			{
			 if(nalias.trim().equalsIgnoreCase("")){
				 String sqldelete="delete from gl_webcat where doc_no='"+naliasdocno+"'";
			 	int delete=stmt.executeUpdate(sqldelete);
			 }
			 else{
		 ...
		 else if(!nalias.trim().equalsIgnoreCase(""))
			{
