...
try{
	...
	double purcost=0.0;
	String strfleet="select depr_date from gl_equipmaster where fleet_no="+fleet;
	...
	String strpurch="select coalesce(sum(dramount),0) purchasevalue from eq_assettran asset left join my_account ac on (asset.acno=ac.acno and ac.codeno='EVEH') where ac.codeno='EVEH' and fleet_no="+fleet;
	...
	String straccdepr="select coalesce(sum(dramount),0) accdeprvalue from eq_assettran asset left join my_account ac on (asset.acno=ac.acno and ac.codeno='EVAD') where ac.codeno='EVAD' and fleet_no="+fleet;
	...
	/* String strasset="select (select sum(dramount) from gc_assettran where fleet_no="+fleet+") sumamount,datediff('"+docdate+"','"+deprdate+"') daysdiff"; */
	 String strasset="select datediff('"+docdate+"','"+deprdate+"') datediff,round((("+purcost+" * depr/100)/365)*(select datediff) ,2) currentdepr from gl_equipmaster where fleet_no="+fleet; 
	
	Statement stmt=conn.createStatement();
	String strcheckcurdepr="select sum(dramount) amt from eq_assettran asset left join my_account ac on asset.acno=ac.acno where fleet_no="+fleet;
