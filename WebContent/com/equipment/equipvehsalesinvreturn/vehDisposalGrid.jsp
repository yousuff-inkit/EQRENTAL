...
String salesinvdocno=request.getParameter("salesinvdocno")==null?"0":request.getParameter("salesinvdocno");
	...
	datafleet='<%=disposaldao.disposalgridSearch(branch,docno)%>';
        ...
        $(document).ready(function () { 	
            ...
            $('#disposalGrid').on('celldoubleclick', function (event) {
     			...
     			var salesinvdocno=document.getElementById("salesinvdocno").value;
     				if(event.args.datafield=='fleet_no'){
     				    $('#fleetwindow').jqxWindow('open');
     					$('#fleetwindow').jqxWindow('focus');
     					fleetnoSearchContent('fleetSearch.jsp?branch='+document.getElementById("brchName").value+'&salesinvdocno='+salesinvdocno, $('#fleetwindow'));
     				}
