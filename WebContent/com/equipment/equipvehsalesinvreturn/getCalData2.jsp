...
try{
	...
	String strpurch="select coalesce(sum(dramount),0) purchasevalue from eq_assettran asset left join my_account ac on (asset.acno=ac.acno and ac.codeno='VEH') where ac.codeno='VEH' and fleet_no="+fleet;
	...
	String straccdepr="select coalesce(sum(dramount),0) accdeprvalue from eq_assettran asset left join my_account ac on (asset.acno=ac.acno and ac.codeno='VAD') where ac.codeno='VAD' and fleet_no="+fleet;
	...
	/* String strasset="select (select sum(dramount) from eq_assettran where fleet_no="+fleet+") sumamount,datediff('"+docdate+"','"+deprdate+"') daysdiff"; */
