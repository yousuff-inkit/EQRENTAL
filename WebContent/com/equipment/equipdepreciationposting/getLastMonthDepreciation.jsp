...
<%	
	...
	try{
		...
		String sqls = "select DATE_FORMAT(min(date),'%d-%m-%Y') deprdates from (select v.depr_date date from gl_equipmaster v left join eq_assettran g on " 
		...
		if(dateDiff.equalsIgnoreCase("0")){
		   ...
		   String strSql = "select v.fleet_no,v.depr_date deprdate,DATEDIFF(last_day('"+sqlDate+"'),min(v.depr_date)) deprDiff,(DATEDIFF(last_day('"+sqlDate+"'),"
		    		 + "min(v.depr_date))) dateDiff,DAY(LAST_DAY('"+sqlDate+"')) noofdays from gl_equipmaster v left join eq_assettran g on v.fleet_no=g.fleet_no where "
