...
System.out.println("INSIDE DELIVERY JSP"+doc+fleet+date+time+km+fuel+location+branch+trancode);
	...
	String testtout="";
	int testdocno=0;
	String strSql22="select dout,tout,fout,kmout,doc_no from gl_emove where fleet_no='"+fleet+"' and status='OUT' ";
...
" cast(concat('"+testdout+"',' ','"+testtout+"')as datetime)ts_dout from gl_emove where doc_no='"+testdocno+"') m";
	...
	String strSql = "update gl_emove set din='"+date1+"',tin='"+time+"',kmin='"+km+"',fin='"+fuel+"',status='IN',trancode='DL',ibrhid='"+branch+"',ilocid='"+location+"',"+
	...
	String test1="select max(doc_no)+1 docNo from gl_emove";
	String test2="select max(doc_no) parentdoc from gl_emove where fleet_no="+fleet;
	...
	String sqlcollectin="select tin tin1,din din1 from gl_emove where doc_no='"+testparent+"'";
	...
	String sqlcollecttimediff="select TIMESTAMPDIFF(SECOND,ts_dout,ts_din)/60 tmin from (select  cast(concat('"+din1+"',' ','"+tin1+"') as datetime) ts_din,"+
			" cast(concat('"+date1+"',' ','"+time+"')as datetime)ts_dout from gl_emove where doc_no='"+testparent+"') m";
	...
	String strsql2="insert into gl_emove(doc_no,date,fleet_no,rdocno,rdtype,trancode,status,parent,dout,tout,kmout,fout,obrhid,olocid,tideal,emp_id,emp_type)values('"+testdoc+"','"+date1+"',"+
	...
	if(rs>0){
		Statement stmtnrm=conn.createStatement();
			String strnrmupdate="update eq_nrm set delivery=1 where doc_no="+doc;
