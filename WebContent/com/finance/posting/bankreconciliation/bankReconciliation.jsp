...
<script type="text/javascript">
	...
	function funloadappliedgrid(){
		$("#jqxBankReconciliation").jqxGrid({ disabled: false});
		  ...
		  var docno = document.getElementById("docno").value;
		  var mode = document.getElementById("mode").value;
		  var fieldbrch = document.getElementById("cmbbranch").value;
		  var check = 1;
		  
		  $("#overlay, #PleaseWait").show();
		  
		  $("#jqxBankReconciliationGrid").load('bankReconciliationGrid.jsp?accountno='+accId+'&date='+date+'&docno='+docno+'&mode='+mode+'&check='+check+'&fieldbrch='+fieldbrch); 
	}
		...
		if (($("#mode").val() == "view") && $("#docno").val()!="") {
		 	...
		 	var dtype=$('#formdetailcode').val();
				   ...
				   $.messager.confirm('Confirm', 'Do you want to have header?', function(r){
					if (r){
						 var win= window.open(reurl[0]+"printBankReconciliation?docno="+document.getElementById("docno").value+"&branch="+document.getElementById("brchName").value+"&dtype="+dtype+"&header=1","_blank","top=150,left=250,Width=1020,Height=500,location=no,scrollbars=no,toolbar=yes");
					     win.focus();
					 }
					else{
						var win= window.open(reurl[0]+"printBankReconciliation?docno="+document.getElementById("docno").value+"&branch="+document.getElementById("brchName").value+"&dtype="+dtype+"&header=0","_blank","top=150,left=250,Width=1020,Height=500,location=no,scrollbars=no,toolbar=yes");
					    win.focus();
					}
				   });
	     }
...
<div class='modern-ui hidden-scrollbar'>
    ...
    <div class="middle-panel">
        ...
        <div class="field-row" style="margin-bottom:0;">
            ...
            <select id="cmbbranch" name="cmbbranch" style="width:130px;" value='<s:property value="cmbbranch"/>'>
                <option></option>
            </select>
            ...
            <i><b><label id="lblformposted" name="lblformposted" style="font-size: 1px;font-family: Tahoma; color:#E0ECF8"><s:property value="lblformposted"/></label></b></i>

            <label class="lbl-right" style="width:70px; margin-left:auto;">Doc No</label>
            <input type="text" id="docno" name="txtbankreconciliationdocno" value='<s:property value="txtbankreconciliationdocno"/>' tabindex="-1" style="width:120px;" />
        </div>
    </div>
    ...
    <div class="middle-panel">
        ...
        <div class="field-row" style="margin-bottom:0;">
            ...
            <div class="input-search-container" style="width: 130px;">
                ...
                <svg class="magnifier-icon" onclick="var d=$('#jqxBankReconciliationDate').jqxDateTimeInput('getDate'); $('#maindate').jqxDateTimeInput('val', d); accountSearchContent('<%=contextPath%>/com/finance/accountsDetailsSearch.jsp?date='+d); $('#txtforsearch').val(2); $('#jqxBankReconciliation').jqxGrid('clear');" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>
            </div>
            <input type="text" id="txtaccname" name="txtaccname" value='<s:property value="txtaccname"/>' style="flex:1;" />
    ...
    <div class="middle-panel">
        <span class="middle-panel-title">Balances</span>
        <div class="field-row" style="margin-bottom:0;">
            <label class="lbl-right" style="width:100px;">Book Balance</label>
            <input type="text" id="txtbookbalance" name="txtbookbalance" style="width:130px; text-align:right;" value='<s:property value="txtbookbalance"/>' tabindex="-1" />

            <label class="lbl-right" style="width:150px;">Uncleared Payments(+ve)</label>
            <input type="text" id="txtunclrpayments" name="txtunclrpayments" style="width:130px; text-align:right;" value='<s:property value="txtunclrpayments"/>' tabindex="-1" />

            <label class="lbl-right" style="width:150px;">Uncleared Receipts(-ve)</label>
            <input type="text" id="txtunclrreceipts" name="txtunclrreceipts" style="width:130px; text-align:right;" value='<s:property value="txtunclrreceipts"/>' tabindex="-1" />

            <label class="lbl-right" style="width:100px;">Bank St. Balance</label>
            <input type="text" id="txtbankbalance" name="txtbankbalance" style="width:130px; text-align:right;" value='<s:property value="txtbankbalance"/>' tabindex="-1" />
        </div>
    </div>
