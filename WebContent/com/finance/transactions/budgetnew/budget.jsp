			...
			if (($("#mode").val() == "view") && $("#docno").val()!="") {
			     ...
			     $("#docno").prop("disabled", false);
					   ...
					   $.messager.confirm('Confirm', 'Do you want to have header?', function(r){
						if (r){
							 var win= window.open(reurl[0]+"printBudgetNew?docno="+document.getElementById("docno").value+"&branch="+document.getElementById("brchName").value+"&header=1","_blank","top=150,left=250,Width=1020,Height=500,location=no,scrollbars=no,toolbar=yes");
						     win.focus();
						 }
						else{
							var win= window.open(reurl[0]+"printBudgetNew?docno="+document.getElementById("docno").value+"&branch="+document.getElementById("brchName").value+"&header=0","_blank","top=150,left=250,Width=1020,Height=500,location=no,scrollbars=no,toolbar=yes");
						    win.focus();
						}
					   });
		     }
	  ...
	  function funValid(){
	   			...
	   			x.onreadystatechange = function() {
	   				if (x.readyState == 4 && x.status == 200) {
	   					...
	   					if(parseInt(items)>0)
	   						{
	   						$.messager.alert('Message','Budget already exists for this time period....!','warning');
	   						}
	   					else
	   						{
	   						funGridLoad();   
	   			...
	   			x.open("GET", "getvalidcheck.jsp?assessmentyear=01."+$('#assessmentYearDate').val(), true);
	   			x.send();
	   		}
...
<form id="frmBudget" action="budgetAction" method="post" autocomplete="off">
...
<table width="100%">
  ...
  <tr>
    ...
    <td colspan="2" align="center"><button class="myButton" type="button" id="btnGridLoad" name="btnGridLoad" onclick="funValid();">Submit</button></td>
  </tr>
</table>
