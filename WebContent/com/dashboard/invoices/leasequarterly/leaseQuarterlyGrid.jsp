 ...
 <%ClsLeaseQuarterlyInvoiceDAO rentaldao=new ClsLeaseQuarterlyInvoiceDAO();%>
...
{ 
	if(mode1=="1"){
		...
		invoicedata='<%=rentaldao.getLeaseQuarterlyInvoice(temp,desc1,date1,branch,client,mode)%>';	 
	}
	else{
...
$(document).ready(function () {
    ...
    $("#leaseQuarterlyGrid").on("bindingcomplete", function (event) {
    	$("#overlay, #PleaseWait").hide();
    	getNoSalikConfig();
//    	$('#leaseQuarterlyGrid').jqxGrid({ sortable: true});
    	});
...
$('#leaseQuarterlyGrid').on('rowselect', function (event) 
    		{
    		    // event arguments.
    		    var args = event.args;
    		    // row's bound index.
    		    ...
    		    var todate=$('#leaseQuarterlyGrid').jqxGrid('getcellvalue',rowBoundIndex, "todate");
    		  	var docdateval=funDateInPeriodNew(todate);
    		    if(docdateval==0){
    		    	$('#leaseQuarterlyGrid').jqxGrid('unselectrow', rowBoundIndex);
    		    }
    		    var duedatecal=$('#leaseQuarterlyGrid').jqxGrid('getcellvalue',rowBoundIndex, "duedatecal");
    		    if(duedatecal==1){
    		    	$('#leaseQuarterlyGrid').jqxGrid('unselectrow', rowBoundIndex);
   ...
   if(document.getElementById("btninvoicesave").style.display=="block"){
	   $('#leaseQuarterlyGrid').jqxGrid({ height: 535 });
   }
});
