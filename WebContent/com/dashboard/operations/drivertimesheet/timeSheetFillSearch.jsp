...
<script type="text/javascript">
	...
	$(document).ready(function () { 
		...
		$('#txtselecteddrname').dblclick(function(){
			driverSearchContent("driverDetailMultiSearch.jsp");
	     });
		
		$('#txtselectedagreement').dblclick(function(){
	...
	function getAgreementData(event){
        var x= event.keyCode;
        if(x==114){
        	$('#txtorgridclick').val('3');
        	agreementNoSearchContent("agreementDetailSearch.jsp");
	...
	function funUpdate(){
	    ...
	    for (var m = 0; m < rows.length; m++) {
	    	...
	    	if(selectedrows[n]==m){
		        ...
		        $('#timeSheetDetailsGridID').jqxGrid('setcellvalue',rr.length-1,"drid",rows[m].drid);
		        $('#timeSheetDetailsGridID').jqxGrid('setcellvalue',rr.length-1,"drname",rows[m].drname);
		        $('#timeSheetDetailsGridID').jqxGrid('setcellvalue',rr.length-1,"agmtvoc_no",rows[m].agmtvoc_no);
		        $('#timeSheetDetailsGridID').jqxGrid('setcellvalue',rr.length-1,"agmtdoc_no",rows[m].agmtdoc_no);
	...
	function funSetFillGrid(){
		...
		var selectedagreement=$("#txtselectedagreement").val();
		var selectedagreementid=$("#txtselectedagreementid").val();
		var selecteddrdocno=$("#txtselecteddrdocno").val();
		var selecteddrname=$("#txtselecteddrname").val();
		...
		$("#overlay, #PleaseWait").show();
		$("#refsearch").load("timeSheetFillSearchGrid.jsp?fillfromdate="+fillfromdate+"&filltodate="+filltodate+"&daysselected="+daysselected+"&fillintime="+intime+"&fillouttime="+outtime+"&fillnormalhrs="+fillnormalhrs+"&fillothrs="+fillothrs+"&selectedagreement="+selectedagreement+"&selectedagreementid="+selectedagreementid+"&selecteddrdocno="+selecteddrdocno+"&gridload=1");
...
<%-- <table width="100%">
  ...
  <tr>
    ...
    <td><input type="text" name="txtselecteddrname" id="txtselecteddrname" placeholder="Press F3 to Search" style="width:100%;height: 20px;" readonly  onkeydown="getFillEmployeeId(event);" value='<s:property value="txtselecteddrname"/>'>
    <input type="hidden" name="txtselecteddrdocno" id="txtselecteddrdocno" style="width:100%;height: 20px;" readonly value='<s:property value="txtselecteddrdocno"/>'></td>
...
<table width="100%">
  <tr>
    ...
    <td width="4%" align="right" style="font-size:9px;">Agreement</td>
    <td width="32%"><input type="text" name="txtselectedagreement" id="txtselectedagreement" placeholder="Press F3 to Search" style="width:80%;height: 20px;" readonly onkeydown="getAgreementData(event);" value='<s:property value="txtselectedagreement"/>'>
    <input type="hidden" name="txtselectedagreementid" id="txtselectedagreementid" style="width:100%;height: 20px;" readonly value='<s:property value="txtselectedagreementid"/>'>
    <input type="hidden" id="txtdaysselected" name="txtdaysselected" style="width:100%;height: 20px;" readonly value='<s:property value="txtdaysselected"/>'/></td>
    <td width="3%" align="right" style="font-size:9px;">&nbsp;</td>
    <td colspan="5">&nbsp;</td>
  ...
  <tr>
    <td align="right" style="font-size:9px;">To</td>
    <td><div id="timeSheetFillToDate" name="timeSheetFillToDate" onchange="funRestrictFillToDate();" value='<s:property value="timeSheetFillToDate" />'></div></td>
    <td align="right" style="font-size:9px;">Driver</td>
    <td><input type="text" name="txtselecteddrname" id="txtselecteddrname" placeholder="Press F3 to Search" style="width:100%;height: 20px;" readonly  onkeydown="getFillEmployeeId(event);" value='<s:property value="txtselecteddrname"/>'>
    <input type="hidden" name="txtselecteddrdocno" id="txtselecteddrdocno" style="width:100%;height: 20px;" readonly value='<s:property value="txtselecteddrdocno"/>'></td>
    ...
    <td width="2%" align="right" style="font-size:9px;">&nbsp;</td><!-- ot -->
    <td width="7%"><div style="visibility: hidden;" id="timeSheetFillOTHrs" name="timeSheetFillOTHrs" value='<s:property value="timeSheetFillOTHrs" />'></div></td>
