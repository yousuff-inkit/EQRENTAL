<%@page import="com.dashboard.audit.rentalagentuserlink.ClsRentalAgentUserLink" %>
<%
	ClsRentalAgentUserLink DAO=new ClsRentalAgentUserLink();
%>
...
<script type="text/javascript">
      ...
      var temp='<%=check%>';
        ...
        $(document).ready(function () {
        	...
        	$("#rentalUserLinkGridID").on("bindingcomplete", function (event) {
        		if(document.getElementById("rdlinking").checked==true){
        			$('#rentalUserLinkGridID').jqxGrid('hidecolumn', 'user_name');
        		} else if(document.getElementById("rddelete").checked==true){
        			$("#rentalUserLinkGridID").jqxGrid('showcolumn', 'user_name');
       		 }
        	});
        	
        	var dataAdapter = new $.jqx.dataAdapter(source);
            
            $("#rentalUserLinkGridID").jqxGrid(
            {
                width: '98%',
                height: 480,
                source: dataAdapter,
            ...
            if(temp=='NA'){
                $("#rentalUserLinkGridID").jqxGrid("addrow", null, {});
            }
            
            $("#overlay, #PleaseWait").hide();
            
            $('#rentalUserLinkGridID').on('rowdoubleclick', function (event) {
                var rowindex1 = event.args.rowindex;
                if(document.getElementById("rdlinking").checked==true){
                	document.getElementById("txtsalesmanid").value = $('#rentalUserLinkGridID').jqxGrid('getcellvalue', rowindex1, "docno");
                	$('#btnlinking').attr("disabled",false);$('#btnremovelinking').attr("disabled",true);
                	var values= $('#rentalUserLinkGridID').jqxGrid('getcellvalue',rowindex1, "salesmaninfo");
...
<div id="rentalUserLinkGridID"></div>
