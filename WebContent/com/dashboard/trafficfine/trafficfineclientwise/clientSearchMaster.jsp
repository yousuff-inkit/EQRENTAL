...
<link href="<%=contextPath%>/css/body.css" media="screen" rel="stylesheet" type="text/css" />
	...
	$(document).ready(function () {
 	...
 	function loadSearch() {
 		...
 		var clname=document.getElementById("Cl_name").value.replace(/ /g, "%20");
 		var mob=document.getElementById("Cl_mob").value.replace(/ /g, "%20");
 		var lcno=document.getElementById("dr_Licence").value.replace(/ /g, "%20");
 		var passno=document.getElementById("dr_Passport").value.replace(/ /g, "%20");
 		var nation=document.getElementById("dr_Nation").value.replace(/ /g, "%20");
	...
	function getdata(clname,mob,lcno,passno,nation,dob,branch){
		 ...
		 $("#clientsearchdiv").load('clientSearchGrid.jsp?clname='+clname+'&mob='+mob+'&lcno='+lcno+'&passno='+passno+'&nation='+nation+'&dob='+dob+'&branch='+branch+'&id=1');
