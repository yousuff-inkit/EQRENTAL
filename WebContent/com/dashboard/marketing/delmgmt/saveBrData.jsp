...
String endkm=request.getParameter("endkm")==null?"":request.getParameter("endkm").toString();
String endfuel=request.getParameter("endfuel")==null?"":request.getParameter("endfuel").toString();
...
String vendordocno=request.getParameter("vendordocno")==null?"0":request.getParameter("vendordocno").toString();
...
try{
	...
	java.sql.Date sqldate=null,sqlvendorinvdate=null,sqlenddate=null;
	if(!date.equalsIgnoreCase("")){
		sqldate=objcommon.changeStringtoSqlDate(date);
	}
	if(!enddate.equalsIgnoreCase("") && doneby.equalsIgnoreCase("1")){
		sqlenddate=objcommon.changeStringtoSqlDate(enddate);
	}
	if(!vendorinvdate.equalsIgnoreCase("")){
	...
	if(sqlvendorinvdate!=null){
		strdel="insert into eq_deldetails(doneby,doctype,deldocno,vndid, invno, amount, vat, netamt, fleet, driver, date, time, km, fuel, address, remarks,brhid, userid,vendorinvdate)values("+doneby+",'PIK',"+deldocno+","+vendordocno+",'"+vendorinvno+"',"+vendoramt+","+vendorvatamt+","+vendornetamt+",'"+fleetname+"','"+drivername+"','"+sqldate+"','"+time+"',"+km+","+fuel+",'"+address+"','"+remarks+"',"+sessionbranch+","+userid+",'"+sqlvendorinvdate+"')";
	}
	else{
		if(sqlenddate!=null){
			strdel="insert into eq_deldetails(doneby,doctype,deldocno,fleet, driver, date, time, km, fuel, address, remarks,brhid, userid,enddate,endtime,endkm,endfuel)values("+doneby+",'PIK',"+deldocno+",'"+fleetname+"','"+drivername+"','"+sqldate+"','"+time+"',"+km+","+fuel+",'"+address+"','"+remarks+"',"+sessionbranch+","+userid+",'"+sqlenddate+"','"+endtime+"',"+endkm+","+endfuel+")";
		}
		else{
			strdel="insert into eq_deldetails(doneby,doctype,deldocno,fleet, driver, date, time, km, fuel, address, remarks,brhid, userid)values("+doneby+",'PIK',"+deldocno+",'"+fleetname+"','"+drivername+"','"+sqldate+"','"+time+"',"+km+","+fuel+",'"+address+"','"+remarks+"',"+sessionbranch+","+userid+")";	
		}
	}
	...
	for(int i=0;i<gridarray.size();i++){
		...
		System.out.println(strgetcontractdet);
		...
		if(mode.equalsIgnoreCase("1")){
			...
			/* if(doneby.equalsIgnoreCase("3")){
				sqlfilters1+=" deldocno="+deldocno+",vendordocno="+vendordocno+",vendorinvno='"+vendorinvno+"',vendoramt="+vendoramt+",vendorvatamt="+vendorvatamt+",vendornetamt="+vendornetamt+",";
			}
			...
			strupdatecalc="update gl_rentalquotecalc set "+sqlfilters1+" address='"+address+"',delstartdate='"+sqldate+"',delstarttime='"+time+"',doneby="+doneby+",delmode="+mode+",delfleetname='"+fleetname+"',drivername='"+drivername+"' where doc_no="+calcdocno;
		...
		if(doneby.equalsIgnoreCase("1") && mode.equalsIgnoreCase("1")){
			...
			if(i==gridarray.size()-1){
				String strinsertmov="insert into gl_vmove(date,rdocno,rdtype,fleet_no,trancode,status,parent,repno,exchng,dout,tout,kmout,fout,obrhid,olocid,oreason,emp_id,emp_type,din,tin,kmin,fin,ibrhid,ilocid,ireason)values"+
				" (CURDATE(),"+docno+",'ERC',"+fleetno+",'DL','OUT',0,0,0,'"+sqldate+"','"+time+"',"+km+","+fuel+","+brhid+","+locid+",'"+remarks+"',"+drvdocno+",'DRV','"+sqlenddate+"','"+endtime+"',"+endkm+","+endfuel+","+brhid+","+locid+",'"+remarks+"')";
		...
		//Updating gl_rentquotecalc pickupdocno
		String strupdateequip="update gl_rentalquotecalc set delpickupdocno="+deldocno+" where doc_no="+calcdocno;
		int updateequip=stmt.executeUpdate(strupdateequip);
		if(updateequip<=0){
			errorstatus=1;	
		}
