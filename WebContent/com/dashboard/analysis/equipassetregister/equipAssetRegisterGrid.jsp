...
<script type="text/javascript">
	  ...
	  var temp2='<%=toDate%>';
	  	...
	  	if(temp!='NA'){ 
	  		data='<%=cva.equipAsssetGridLoading(branchval, fromDate, toDate, fleetNo, group, model,rptType, check)%>';
	  		<%-- var dataExcelExport='<%=cva.equipAsssetExcelExport(branchval, fromDate, toDate, fleetNo, group, model,rptType, check)%>'; --%>
	  	}
        $(document).ready(function () {
            ...
            if(temp=='NA'){
                $("#equipAssetGrid").jqxGrid("addrow", null, {});
            }

			$("#overlay, #PleaseWait").hide();
			
			
			 $('#equipAssetGrid').on('rowdoubleclick', function (event) {
				    var rowindex=event.args.rowindex;
	                var desc= "Equipment Asset Register"; 
	                var fleetno=$('#equipAssetGrid').jqxGrid('getcellvalue', rowindex, "fleet_no");
          	        var url=document.URL;
					var reurl=url.split("com/");
					
					var detName=$('#equipAssetGrid').jqxGrid('getcellvalue', rowindex, "flname");
					var path="com/dashboard/analysis/equipassetregister/equipDetailedAsset.jsp";
					/* top.addTab( detName,reurl[0]+""+path+"?name="+encodeURIComponent(detName)+"&main="+encodeURIComponent(desc)+"&branchval="+temp+'&fleetno='+fleetno+'&fromdate='+temp1+'&todate='+temp2);
	          	  */ 
