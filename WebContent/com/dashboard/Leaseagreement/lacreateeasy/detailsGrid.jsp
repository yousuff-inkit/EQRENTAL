...
ClsLACreateEasyDAO ladao=new ClsLACreateEasyDAO();
%> 
...
$(document).ready(function () { 
	...
	var source = 
            ...
            { 
                ...
                columns: [
                          ...
                          { text: 'SL#', sortable: false, filterable: false, editable: false,
							...
							{ text: 'Fleet No', datafield: 'fleet_no', width: '6%', editable: false },
        ...
        $("#detailsgrid").on("rowdoubleclick", function (event) 
        		{
        		    ...
        		    $('#fleetno').val($('#detailsgrid').jqxGrid('getcellvalue',rowBoundIndex,'fleet_no'));
        		    funGetFleetMovInfo($('#fleetno').val());
		...
		function funGetFleetMovInfo(fleetno){
			var x = new XMLHttpRequest();
			x.onreadystatechange = function() {
				if (x.readyState == 4 && x.status == 200) {
					items = x.responseText;
					items=items.trim();
					document.getElementById("kmout").value=items.split("::")[0];
		            document.getElementById("cmbfuelout").value=items.split("::")[1];
					} else {
				}
			}
			x.open("GET", "getFleetMovInfo.jsp?fleetno="+fleetno, true);
			x.send();
		}
    </script>
