   <%@page import="com.dashboard.vehicle.unrentableremarks.ClsUnrentableDAO"%>
     <%ClsUnrentableDAO cmd= new ClsUnrentableDAO(); %>
	...
	if(temp1!='NA')
{
	...
	var temp1='<%=barchval%>';
	 ...
	 var leasedata;
	 if(temp1!='NA'){
		 leasedata= '<%=cmd.subDetailss(barchval) %>';
		 bb=0;
	 }
	 else{
		 leasedata=[];
...
$(document).ready(function () {
    ...
    $('#statusdetails').on('rowdoubleclick', function (event) 
    	 ...
    	 $("#overlay, #PleaseWait").hide();     	
		    ...
		    $("#fleetdiv").load("unRentableremarksGrid.jsp?relodestatus="+relodestatus.replace(/ /g, "%20")+"&branch="+'<%=barchval%>');
