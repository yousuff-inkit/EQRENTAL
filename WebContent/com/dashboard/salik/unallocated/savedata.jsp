			...
			for(int i=0;i<rowValues.size();i++)
	            ...
	            if(blockreplaced==1){
	            	...
	            	String sqlup1=" update  gl_salik s left join gl_vehmaster v1 on v1.salik_tag=s.tagno "
	            		...
	            		+" where s.sal_date<='"+uptosqldate+"' and s.isallocated=0 and s.status in (0,3)  and v1.fleet_no is not null   "+sqltest+" "+sqlexcludefleets;
	        	System.out.println("==== sqlup1 === "+sqlup1);
	            		stmt.executeUpdate(sqlup1);
	            
	 	String sqlup="update gl_salik s  left join gl_vehmaster veh on veh.salik_tag=s.tagno "+
				 				" set reason='Fleet Not Recognize' where  s.sal_date<='"+uptosqldate+"' and isallocated=0 and s.status in (0,3)  and veh.fleet_no is null ";
