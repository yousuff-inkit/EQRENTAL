	...
	<nav class="navbar navbar-default navbar-fixed-top travel-navbar">
  		<div class="container-fluid">
    		<div class="navbar-header">
      			...
      			<a class="navbar-brand m-t-6" href="#">Travel Desk</a>
    		</div>
                    					...
                    					<div class="tab-pane active" role="tabpanel" id="step1">   
											...
											<div class="newclientinfo">
												<div class="row m-t-10">
								        			...
								        			<div class="col-xs-12 col-sm-12 col-md-12 col-lg-4">
								        				<label class="radio-inline" for="trrdet"><input type="radio" id="trrdet" name="stkledtr" onchange="funchecktour();" value="trrdet" class="optclienttype">Walk In Client</label>
								        	...
								        	<div class="row">
							        			...
							        			<div class="col-xs-12 col-sm-6 col-md-4 col-lg-1">
							        				<div class="form-group">
							        					...
							        					<input type="text" name="tourdiscountchild" id="tourdiscountchild" class="form-control input-sm text-right" onchange="funvalidatechilddis();">
							        				</div>
							        			</div>
							        			<div class="col-xs-12 col-sm-6 col-md-4 col-lg-1"> 
							        				<div class="form-group">   
							        					<label>Discount Adult</label>
							        					<input type="text" name="tourdiscountadult" id="tourdiscountadult" class="form-control input-sm text-right" onchange="funvalidateadultdis();">
							        				</div>
							        			</div>
							        			<div class="col-xs-12 col-sm-6 col-md-4 col-lg-1">
							        				<div class="form-group">
...
function setTourGrid(){    
   ...
   var x=new XMLHttpRequest();
		...
		x.open("GET","savetourData.jsp?gridarray="+encodeURIComponent(gridarray)+"&limodocno="+limodocno+"&rdocno="+rdocno+"&brhid="+branchid+"&edate="+$('#trdate').jqxDateTimeInput('val')+"&userid="+userid+"&clientid="+client+"&client="+encodeURIComponent(refname)+"&mob="+mobno+"&mail="+email+"&locid="+locid+"&type="+clienttype+"&nationid="+nationid+"&sadocno="+salesagentdocno+"&sacno="+salesagentacno+"&salid="+$('#salesmanidtr').val()+"&vocno="+$('#hidvocno').val()+"&agentid="+agentid,true);                    
		x.send();  
   }
	/* Tourgrid ends*/      
  ...
  if($('#tourismconfig').val()!=1){
	...
	function funinvoicecheck(){  
		  ...
		  var x=new XMLHttpRequest();
			...
			items = x.responseText;  
				        if(parseInt(items)==0){  
				        	swal({
				    			type: 'warning',
				    			title: 'Warning',     
				    			text: 'Please create commercial receipt...'         
				    			});
				    	    }else{
				    	    	invoiceValidatetr();                
				    	    }
			...
			x.open("GET","getconfirminvoice.jsp?rdocno="+rowno,true);                           
			x.send(); 
	}
...
function funRoundAmt(value,id){
  ...
  function funVoucherPrinttr(){   
	      ...
	      var reurl=url.split("travelnew.jsp");      
	      var win= window.open(reurl[0]+"printtourvoucherreceipt?docno="+$('#txtrdocno').val()+"&branch="+document.getElementById("cmbbranch").value,"_blank","top=150,left=250,Width=1020,Height=500,location=no,scrollbars=no,toolbar=yes");
	      win.focus();           
		}
 /*  Voucher Receipt Print Ends*/
  ...
  function funXOPrinttr(){     
		  var url=document.URL;  
	      var reurl=url.split("travelnew.jsp");      
	      var win= window.open(reurl[0]+"printtourxo?docno="+$('#txtrdocno').val()+"&branch="+document.getElementById("cmbbranch").value,"_blank","top=150,left=250,Width=1020,Height=500,location=no,scrollbars=no,toolbar=yes");
	      win.focus();           
		}
 /*  XO Print Ends*/  
