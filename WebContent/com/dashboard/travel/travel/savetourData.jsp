...
<%   
	...
	try{
		...
		if(rdocno==0){
			...
			String strsql="insert into tr_servicereqm(voc_no, date, Cldocno, userid, status, brhid, refname, mob, mail, locid, type, nationid, salid, agentid) values('"+vocno+"','"+sqldate+"',"+clientid+","+userid+",3,"+brhid+",'"+client+"','"+mobno+"','"+email+"','"+locid+"',"+type+",'"+nationid+"','"+salid+"','"+agentid+"')";   
		...
		//System.out.println("in=="+docno);
			...
			if(val>0){ 
				...
				dat=DAO.insert(cdate, clientid, "0",client, mobno, "1",session, "A","LBK",brhid,"Service Request Tour - "+vocno,airport,flight,hotel,roomno,locationtype);          
				//System.out.println(rdocno+"===Limo insert>>>"+dat); 
				if(dat>0){             
					tempval2=DAO.insertTarif(dat, transferarray, hoursarray,session,"A");                      
				}
				}else{    
					test=DAO.edit(Integer.parseInt(limodocno), cdate, clientid, "0",client, mobno, "1",session, "E","LBK",brhid,"Service Request Tour - "+vocno,airport,flight,hotel,roomno,locationtype);          
