...
try{
	...
	if(rdocno==0){
		...
		for(int i=0;i<newarray.size();i++){   
			...
			if((!temp[1].trim().equalsIgnoreCase("undefined") && !temp[1].trim().equalsIgnoreCase("NaN") && !temp[1].trim().equalsIgnoreCase("")) || (!temp[2].trim().equalsIgnoreCase("undefined") && !temp[2].trim().equalsIgnoreCase("NaN") && !temp[2].trim().equalsIgnoreCase(""))){
				
			String sertypeid = (temp[0].trim().equalsIgnoreCase("undefined") || temp[0].trim().equalsIgnoreCase("NaN") || temp[0].trim().equalsIgnoreCase("") || temp[0].trim().isEmpty()?"0":temp[0].trim()).toString();
			String remarks = (temp[1].trim().equalsIgnoreCase("undefined") || temp[1].trim().equalsIgnoreCase("NaN") || temp[1].trim().equalsIgnoreCase("") || temp[1].trim().isEmpty()?"0":temp[1].trim()).toString();
			String pax = temp[2].trim().equalsIgnoreCase("undefined") || temp[2].trim().equalsIgnoreCase("NaN") || temp[2].trim().equalsIgnoreCase("") || temp[2].trim().isEmpty()?"0":temp[2].trim().toString();
			...
			if(rowsno>0){    
				String sql="update tr_servicereqd set srno="+(i+1)+", servid="+sertypeid+", remarks='"+remarks+"', pax="+pax+" where rowno="+rowsno+"";
				//System.out.println(sql);         
				 val=stmt.executeUpdate(sql);  
			}else{
			String sql="insert into tr_servicereqd(rdocno, srno, servid, remarks, pax) values("+docno+","+(i+1)+","+sertypeid+",'"+remarks+"',"+pax+")";
			//System.out.println(sql);      
			 val=stmt.executeUpdate(sql);
			}
			if(val>0){
