		 ...
		 while(rss.next())
		 {
			 ...
			 if(rss.getString("dtype").equalsIgnoreCase("INV"))
			 {
				...
				//double ldramounts=0;
			     ...
			     for (int i=0;i<=2;i++)
				   {
					 if(i==0)
					 {
						 ...
						 Statement s6 = conn.createStatement ();
							...
							if(resultSetnw.next()){
								String sqlvehnw2 ="select ROUND(SUM(QTY*COST_PRICE),2) AS TOTAL from my_prddout WHERE TR_NO="+acnos+"";
								ResultSet resultSetnw2 = s6.executeQuery(sqlvehnw2);
								if (resultSetnw2.next()) {
									pricetottal=resultSetnw2.getDouble("TOTAL")+(totalcost*-1);
								}
								 ldramounts=0 ;     
								...
								String sqltst="update my_jvtran set dramount="+pricetottal+",ldramount="+ldramounts+" where tr_no="+trmo+" and acno="+acnos+"";
				 ...
				 else if(i==1)
				 {
					...
					if(resultSetnw.next()){
						String sqlvehnw2 ="select ROUND(SUM(QTY*COST_PRICE),2) AS TOTAL from my_prddout WHERE TR_NO="+acnos2+"";
						ResultSet resultSetnw2 = s8.executeQuery(sqlvehnw2);
						if (resultSetnw2.next()) {
							pricetottal2=resultSetnw2.getDouble("TOTAL")+(totalcost);
						}
						...
						String sqltst="update my_jvtran set dramount="+pricetottal2+",ldramount="+ldramounts2+" where tr_no="+trmo+" and acno="+acnos2+"";
						 val=stmt.executeUpdate(sqltst);
					}
					else{
			 ...
			 else if(rss.getString("dtype").equalsIgnoreCase("GIS"))
			 {
				...
				System.out.println("trno=="+trno);
					...
					if(resultSetnw.next()){
						String sqlvehnw2 ="select ROUND(SUM(QTY*COST_PRICE),2) AS TOTAL from my_prddout WHERE TR_NO="+accounts+"";
						ResultSet resultSetnw2 = s1.executeQuery(sqlvehnw2);
						if (resultSetnw2.next()) {
							dramount=resultSetnw2.getDouble("TOTAL")+(totalcost);
						}
						double ldramount=0;
						...
						String sqltst="update my_jvtran set dramount="+dramount+",ldramount="+ldramount+" where tr_no="+trno+" and acno="+accounts+"";
						 val=stmt.executeUpdate(sqltst);
					}
					else{
						dramount=totalcost;
					...
					ResultSet resultSetnw2 = s1.executeQuery(sqlvehnw2);
					if(resultSetnw2.next()){
						String sqlvehnw3 ="select ROUND(SUM(QTY*COST_PRICE),2) AS TOTAL from my_prddout WHERE TR_NO="+acnos+"";
						ResultSet resultSetnw3 = s1.executeQuery(sqlvehnw3);
						if (resultSetnw3.next()) {
							pricetottal=resultSetnw3.getDouble("TOTAL")+(totalcost*-1);
						}
						 ldramounts=0 ;     
						if(currencytype1.equalsIgnoreCase("D"))
						{
						...
						String sqltst="update my_jvtran set dramount="+pricetottal+",ldramount="+ldramounts+" where tr_no="+trno+" and acno="+acnos+"";
						 val=stmt.executeUpdate(sqltst);
					}
					else{
						pricetottal=totalcost*-1;
									...
									if(tass1181.next()) {
									 ...
									 Statement s11=conn.createStatement();
										...
										if(resultSetnw4.next()){
											String sqlvehnw3 ="select ROUND(SUM(QTY*COST_PRICE),2) AS TOTAL from my_prddout WHERE TR_NO="+accounts+"";
											ResultSet resultSetnw3 = s1.executeQuery(sqlvehnw3);
											if (resultSetnw3.next()) {
												girdramount=resultSetnw3.getDouble("TOTAL")+(totalcost);
											}
											...
											String sqltst="update my_jvtran set dramount="+(girdramount)+",ldramount="+(girldramount)+" where tr_no="+csttrno+" and acno="+accounts+"";
											val=stmt.executeUpdate(sqltst);
										}
										else{
										...
										if(resultSetnw5.next()){
											...
											if (resultSetnw3.next()) {
												girpricetottal=resultSetnw3.getDouble("TOTAL")+(totalcost*-1);
											}
											...
											String sqltst="update my_jvtran set dramount="+(pricetottal)*-1+",ldramount="+(ldramounts)*-1+" where tr_no="+csttrno+" and acno="+acnos+"";
											val=stmt.executeUpdate(sqltst);
										}
										else{
							...
							if(tass1181.next()) {
							 ...
							 Statement s11=conn.createStatement();
								...
								if(resultSetnw5.next()){
									...
									String sqltstnw="update  my_jvtran set dramount="+(girdramount)+",ldramount="+(girldramount)+" where tr_no="+csttrno+" and acno="+accounts+"";
								...
								if(resultSetnw5.next()){
									...
									String sqltstnw="update  my_jvtran set dramount="+(pricetottal)*-1+",ldramount="+(ldramounts)*-1+" where tr_no="+csttrno+" and acno="+acnos+"";
									 val=stmt.executeUpdate(sqltstnw);
								}
								else{
									girpricetottal=totalcost*-1;
