...
<%   String branchval = request.getParameter("branchval")==null?"NA":request.getParameter("branchval").trim();
	 ...
	 String loadInBaseCurr = request.getParameter("chkloadinbasecurr")==null?"0":request.getParameter("chkloadinbasecurr");
...
<script type="text/javascript">
      ...
      var l5='>='+$('#txtlevel5from').val()+'';
	  	...
	  	if(temp!='NA'){ 
	  		   data='<%=DAO.ageingStatement(branchval, upToDate, atype, accdocno, salesperson, category, clientStatus, level1From, level1To, level2From, level2To, level3From, level3To, level4From, level4To, level5From, loadInBaseCurr, check)%>';
	  	}
        ...
        $(document).ready(function () {
        	...
        	var source =
            {
        	 ...
        	 $("#ageingStatement").on("bindingcomplete", function (event) {
   		    	$('#ageingStatement').jqxGrid('hidecolumn', 'currency');
   			    
   		    	if (parseInt($("#hidmulticurrency").val())>0){
               		$('#ageingStatement').jqxGrid('showcolumn', 'currency');
   			    }
               });
