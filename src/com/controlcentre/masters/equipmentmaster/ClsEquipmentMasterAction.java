...
public class ClsEquipmentMasterAction extends ActionSupport{
	ClsCommon ClsCommon=new ClsCommon();

	ClsEquipmentMasterDAO equipdao= new ClsEquipmentMasterDAO();
	private String cmbcategory,hidcmbcategory,cmbsubcategory,hidcmbsubcategory,cmbsize,hidcmbsize,srno;
	private String tpiexpiry;
	
	
	public String getTpiexpiry() {
		return tpiexpiry;
	}

	public void setTpiexpiry(String tpiexpiry) {
		this.tpiexpiry = tpiexpiry;
	}
	...
	public String getHidcmbsubcategory() {
		return hidcmbsubcategory;
	}

	public void setHidcmbsubcategory(String hidcmbsubcategory) {
	...
	public String getHidcmbsize() {
		return hidcmbsize;
	}

	public void setHidcmbsize(String hidcmbsize) {
		this.hidcmbsize = hidcmbsize;
	}
...
public void setDeleted(String deleted) {
	...
	java.sql.Date sqltpiexpiry=null;

	
	public void getData(ClsEquipmentMasterBean bean){
		setHidcmbcategory(getCmbcategory());
		setHidcmbsubcategory(getCmbsubcategory());
		setHidcmbsize(getCmbsize());
		setSrno(getSrno());
		...
		if(sqltpiexpiry!=null){
			setTpiexpiry(sqltpiexpiry.toString());
		}
...
//		System.out.println("here");
		...
		ClsEquipmentMasterAction masteraction=new ClsEquipmentMasterAction();
		masteraction.setCmbcategory(getCmbcategory());
		masteraction.setCmbsubcategory(getCmbsubcategory());
		masteraction.setCmbsize(getCmbsize());
		masteraction.setSrno(getSrno());
		
		if(mode.equalsIgnoreCase("A") || mode.equalsIgnoreCase("E") || mode.equalsIgnoreCase("R")){
			
			if(!getTpiexpiry().equalsIgnoreCase("") && getTpiexpiry()!=null){
				sqltpiexpiry=ClsCommon.changeStringtoSqlDate(getTpiexpiry());
				masteraction.sqltpiexpiry=sqltpiexpiry;
			}
...
//			System.out.println("%"+getPurchase()+"%"+getFormdetailcode());
			setAststatus("I");
			bean=equipdao.insert(getFleetname(),jqxDate,getCmbauthority(),getCmbplate(),getRegno(),getCmbgroup(),getCmbbrand(),getCmbmodel(),
...
//			System.out.println("Release Location"+getCmbrlsloc());
			...
			int val1=equipdao.release(getReleasefleet(),getCmbrlsbranch(),getCmbrlsloc(),getCurrent_km(),getCmbfuel(),getDocno(),session,mode,getCmbrentalstatus(),reldate,getReleasetime());
			if(purchDate!=null){
				setJqxPurchaseDate(purchDate.toString());
			}
			if(jqxDate!=null){
...
//		System.out.println("not saved");
		setMsg("Not Saved");
			...
			setHidcmbsize(getCmbsize());
			setSrno(getSrno());
			System.out.println("Action TPI Expiry SQL Date:"+masteraction.sqltpiexpiry);
			boolean Status=equipdao.edit(getFleetno(),getFleetname(),jqxDate,getCmbauthority(),getCmbplate(),getRegno(),getCmbgroup(),getCmbbrand(),getCmbmodel(),
					getCmbyom(),getAststatus(),getOpstatus(),getSalik_tag(),getTcno(),getHiddealer(),getDeal_no(),getPurchase(),getLpo_no(),getPurchase_invoice(),
					purchDate,getPurchase_cost(),getAdditions(),getTotal(),getHidfinancier(),getInterest_amt(),getDown_payment(),getNo_installments(),regDate,
					...
					regotherInsDate,getPremium_perc(),getPolicy_no(),getPremium_amt(),getEngine_no(),getVin(),getChasis_no(),getCmbveh_color(),getWarranty_period(),
					warntyFrmDate,warntyToDate,getWarranty_km(),getService_km(),lstserDate,getLast_srvc_km(),getCurrent_km(),getCmbfuel(),getBranded(),getCmbavail_br1(),
					getDocno(),session,getFuelcapacity(),getCmbfueltype(),getFormdetailcode(),specarray,getFileno(),getInsurmember(),getTrackid(),getHidmortgaged(),getCalibrationkm(),masteraction);
			...
			setSrno(getSrno());
			if(Status){
				if(masteraction.sqltpiexpiry!=null){
					setTpiexpiry(masteraction.sqltpiexpiry.toString());
				}
				if(purchDate!=null){
					setJqxPurchaseDate(purchDate.toString());
				}
...
setMortgaged(getMortgaged());setHidmortgaged(getHidmortgaged());
				return "fail";
					...
					int Status=equipdao.delete(getFleetno(),getFleetname(),jqxDate,getCmbauthority(),getCmbplate(),getRegno(),getCmbgroup(),getCmbbrand(),getCmbmodel(),
							getCmbyom(),getAststatus(),getOpstatus(),getSalik_tag(),getTcno(),getHiddealer(),getDeal_no(),getPurchase(),getLpo_no(),getPurchase_invoice(),
							purchDate,getPurchase_cost(),getAdditions(),getTotal(),getHidfinancier(),getInterest_amt(),getDown_payment(),getNo_installments(),regDate,
							relDate,getInstallment_amt(),getTran_no(),getDepr_perc(),getAccu_dep(),regExpDate,getCmbinsurance_type(),getHidinsurance_comp(),getInsured_amt(),
							regotherInsDate,getPremium_perc(),getPolicy_no(),getPremium_amt(),getEngine_no(),getVin(),getChasis_no(),getCmbveh_color(),getWarranty_period(),
...
//			System.out.println("Release Request:"+request.getParameter("fleetno"));
			...
			bean=equipdao.getViewDetails(getFleetno());
//			System.out.println(bean);
			
			System.out.println("getErrormsg==="+bean.getErrormsg());
			setTpiexpiry(bean.getTpiexpiry());
			setHidcmbcategory(bean.getHidcmbcategory());
			setHidcmbsubcategory(bean.getHidcmbsubcategory());
			setHidcmbsize(bean.getHidcmbsize());
			setSrno(bean.getSrno());
			setErrormsg(bean.getErrormsg());
