...
//	System.out.println("sessoin"+session.getAttribute("COMPANYNAME"));
	...
	ClsFinancierAction act=new ClsFinancierAction();
...
//	Date trail=getDate_plateCode();
	...
	act.setFinname(getFinname());
	if(mode.equalsIgnoreCase("A")){
					int val=financierDAO.insert(getFinid(),getFinname(),sqlStartDate,getTxtaccno(),session,getMode(),getFormdetailcode(),act);
					if(val>0.0){
						setFinid(getFinid());
