...
public class ClsVisaAction extends ActionSupport{
	
	ClsCommon ClsCommon=new ClsCommon();
	ClsVisaDAO serviceDAO= new ClsVisaDAO();
	ClsVisaBean bean;
	...
	public String saveAction() throws ParseException, SQLException{
		...
		if(mode.equalsIgnoreCase("A")){
						int val=serviceDAO.insert(getName(),getValidity(), session,getMode(),getFormdetailcode());
						System.out.println(val);
						if(val>0){
							
							setName(getName());
							setValidity(getValidity());
							setMode(getMode());
						    setDocno(val);						    
				...
				int val=serviceDAO.edit(getName(),getValidity(),session,getMode(),getDocno(),getFormdetailcode());
				...
				boolean Status=serviceDAO.delete(getName(),getValidity(),session,getMode(),getDocno(),getFormdetailcode());
			if(Status){
				setName(getName());
				setValidity(getValidity());
				setDocno(getDocno());
