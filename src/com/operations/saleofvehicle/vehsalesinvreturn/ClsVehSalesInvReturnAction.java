...
//			System.out.println("Branch in Action: "+getBrchName());
						ClsVehSalesInvReturnBean bean=disposalDAO.insert(sqlStartDate,getCmbtype(),getDescription(),session,getMode(),disposalarray,
								getFormdetailcode(),getClientacno(),getClient(),getDays(),getBrchName(),request,getMdoc(),getSalesinvdocno(),getSalesinvtrno());
						if(bean.getDocno()>0){
...
//				System.out.println(disposalarray.get(i));
			...
			boolean Status=disposalDAO.edit(sqlStartDate,getCmbtype(),getDescription(),session,getMode(),getDocno(),getTrno(),
					disposalarray,getFormdetailcode(),getClientacno(),getClient(),getBrchName(),getSalesinvdocno(),getSalesinvtrno());
			if(Status){
	...
	public String printAction() throws SQLException{
		HttpServletRequest request=ServletActionContext.getRequest();
		HttpSession session=request.getSession();
		setUrl(objcommon.getPrintPath("VSIR"));
		...
		setLblprintname("Tax Vehicle Sales Invoice Return");
