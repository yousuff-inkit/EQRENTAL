...
public class ClsInspAttach extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		try {
			...
			path = path1.replace("\\", "/");
			// ServletContext context = ((HttpSession)
			...
			String strupdate = "update gl_vinspd set attach='" + fname
					+ "',path='" + path + "/attachment/" + dirname + "/"
					+ fname + "' where rdocno=" + doc + " and rowno=" + srno;
			/*
			 * String strinsert=
			 * "insert into my_fileattach(dtype,doc_no,brhid,sr_no,imgno,date,user,path)values('"
			 * +
			 * code+"',"+doc+",'"+session.getAttribute("BRANCHID").toString()+"',"
			 * +
			 * " "+srno+",'"+fname2+"',CURDATE(),'"+session.getAttribute("USERID"
			 * ).toString()+"','"+path + "/attachment/"+dirname+ "/"+
			 * fname+"')"; // System.out.println("Insert Attach"+strinsert);
			 */
