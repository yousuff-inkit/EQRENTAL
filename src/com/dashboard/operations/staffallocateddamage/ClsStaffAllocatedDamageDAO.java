...
//			  System.out.println("Checkking: "+branch+"///"+fromDate+"////"+toDate+"/////"+type+"////"+employee);
			    ...
			    String strsql="select insp.fine accfines,insp.brhid,INSP.DATE,(select acno from gl_invmode where idno=10) damageacc,SAL.ACC_NO expacc,veh.reg_no regno,insp.doc_no inspno,insp.refdocno refno,insp.type,insp.amount,insp.rfleet fleetno,if(nrm.drid=0,nrm.staffid,nrm.drid) empno,"+
			    		...
			    		" veh on insp.rfleet=veh.fleet_no where insp.invno=0 and (insp.amount>0 or insp.fine>0) and insp.reftype='NRM'"+sqltest;
