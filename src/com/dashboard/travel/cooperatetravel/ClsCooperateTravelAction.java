...
public class ClsCooperateTravelAction {
	    ...
	    java.sql.Date sqltodate = null;   
			 ...
			 try {	
			       ...
			       // System.out.println("in...."+user);
			        ...
			        JasperDesign design = JRXmlLoader.load(request.getSession().getServletContext().getRealPath("com/dashboard/travel/cooperatetravel/voucherreciept.jrxml"));  
	...
	public String printActionXO() throws ParseException, SQLException{        
		...
		String clientname="",imgpath="",comp="",tel="",fax="",brch="",location="",cmpname="",dloc="",ploc="",loctype="",refno="",rname="",refon="",vocno="0";  
		ResultSet rss=null; 
		int conmeth=0;
	    HttpServletResponse response = ServletActionContext.getResponse();
	    java.sql.Date sqlfromdate = null;   
	    java.sql.Date sqltodate = null;   
			 ...
			 try {	
				    ...
				    String strsqls="select coalesce(ser.guest,'') clientname,ser.voc_no,if(tr.loctype='F','Flight No',if(tr.loctype='H','Room No','')) refon,if(tr.loctype='F','Flight',if(tr.loctype='H','Hotel','')) loctype,tr.rname,tr.refno,coalesce(gl.location,'') ploc,coalesce(gls.location,'') dloc,coalesce(tr.tourid,0) tourid,ser.refname,coalesce(ac.trnnumber,'') trnnumber,format(coalesce(round(g.amount,2),0),2) amount,coalesce(g.refno,'') cardno, "
			        ...
			        param.put("serdocno",vocno);  
			        //param.put("client",client);  
			        JasperDesign design = JRXmlLoader.load(request.getSession().getServletContext().getRealPath("com/dashboard/travel/cooperatetravel/XOprint.jrxml"));  
  	                JasperReport jasperReport = JasperCompileManager.compileReport(design);
  	                generateReportPDF(response, param, jasperReport, conn);      
	               } catch (Exception e) {      
	                 e.printStackTrace();
	...
	public String emailAction(String docno,String branch,String amendment) throws ParseException, SQLException{        
		...
		String clientname="",imgpath="",comp="",tel="",fax="",brch="",location="",cmpname="",print="0",ccemail="",dloc="",ploc="",loctype="",refno="",rname="",refon="",vocno="0"; 
		ResultSet rs=null,rss=null;    
	    ...
	    java.sql.Date sqltodate = null;   
			 ...
			 try {	
				    ...
				    while(rsss.next()){ 
                        ...
                        clientname=rsss.getString("clientname"); 
				    }
				    imgpath=request.getSession().getServletContext().getRealPath("/icons/epic.jpg");  
			        imgpath=imgpath.replace("\\", "\\\\");   
			        if(conmeth==1){
