package com.dashboard.travel.cooperatetravel;
...
import net.sf.json.JSONArray;
	public class ClsCooperateTravelDAO {  
		...
		public JSONArray searchVendor(HttpSession session,int tourid,String dates) throws SQLException{
			...
			try{  
				...
				String strsql="select a.* from(select ts.stockid,sum(coalesce(ts.adult,0)) adultcount,sum(coalesce(ts.child,0)) childcount,'stock' type,0 cmode,ts.stockid rowno,concat(u.user_name,' - ',(select coalesce(ac.refname,'') refname from my_account acc left join my_acbook ac on ac.acno=acc.acno where acc.codeno='Stock Vendor')) name,(select coalesce(ac.cldocno,0) cldocno from my_account acc left join my_acbook ac on ac.acno=acc.acno where acc.codeno='Stock Vendor') doc_no,ts.cpadult adult,ts.spadult,ts.cpchild child,ts.spchild,0 admaxdis, 0 chmaxdis from tr_tourstock ts left join my_user u on u.doc_no=ts.userid where ts.tourid='"+tourid+"' and '"+sqltodate+"'<ts.expdate group by ts.tourid)a where (a.adultcount>0 or a.childcount>0) "  
						...
						+ "t.rowno,refname name,doc_no,t.adult,t.spadult,t.child,t.spchild,coalesce(t.copadultdismax,0) admaxdis, coalesce(t.copchilddismax,0) chmaxdis from "
						...
						+ "t.adult,t.spadult,t.child,t.spchild,coalesce(t.copadultdismax,0) admaxdis, coalesce(t.copchilddismax,0) chmaxdis from tr_prtourd t "
						...
						+ "coalesce(t.copadultdismax,0) admaxdis, coalesce(t.copchilddismax,0) chmaxdis from tr_prtourd t inner join my_acbook ac on ac.doc_no=t.vendorid and ac.dtype='vnd'"
						...
						+ "refname name,doc_no,t.adult,t.spadult,t.child,t.spchild,coalesce(t.copadultdismax,0) admaxdis, coalesce(t.copchilddismax,0) chmaxdis from tr_prtourd t inner join "
						...
						+ "union all select ac.cmode,t.rowno,refname name,doc_no,t.adult,t.spadult,t.child,t.spchild,coalesce(t.copadultdismax,0) admaxdis, coalesce(t.copchilddismax,0) chmaxdis from "
						...
						+ "doc_no,t.adult,t.spadult,t.child,t.spchild,coalesce(t.copadultdismax,0) admaxdis, coalesce(t.copchilddismax,0) chmaxdis from tr_prtourd t inner join my_acbook ac on ac.doc_no=t.vendorid and ac.dtype='vnd'"
						...
						+ "t.child,t.spchild,coalesce(t.copadultdismax,0) admaxdis, coalesce(t.copchilddismax,0) chmaxdis from tr_prtourd t inner join my_acbook ac on ac.doc_no=t.vendorid and ac.dtype='vnd' where t.tourid='"+tourid+"' "
