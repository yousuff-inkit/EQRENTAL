...
public class ClsMortgageSettlementDAO {
	...
	public JSONArray getMortgageData(String fromdate,String todate,String id,String purchasedocno) throws SQLException{
		...
		try{
			...
			" convert(coalesce(m.chqno,''),char(50)) chequeno,d.bpvno  from gl_vpurdetd d  left join gl_vpurm pm on pm.doc_no=d.rdocno left join my_unclrchqbm m on m.doc_no=d.bpvno  and m.brhid=pm.brhid"+
	...
	public JSONArray getDeleteData(String purchasedocno,String id) throws SQLException{
		...
		try{
			conn=objconn.getMyConnection();
			Statement stmt=conn.createStatement();
			String strsql="select 1 defaultdeleterow,m.voc_no purchasevocno,r.purchasedocno,r.date,r.chequeno,r.principalamt,r.interestamt,r.amount,r.bpvno from gl_loanrestructure r left join gl_vpurm m on m.doc_no=r.purchasedocno where r.deletestatus=1 and r.purchasedocno="+purchasedocno;
			ResultSet rs=stmt.executeQuery(strsql);
		...
		* moved below
		...
		if(!vehicleremove.equalsIgnoreCase("")){
			...
			for(int i=0;i<purchasearray.size();i++){
				...
				/*sqldate=objcommon.changetstmptoSqlDate(purchasetemp[3].substring(0,26));*/
