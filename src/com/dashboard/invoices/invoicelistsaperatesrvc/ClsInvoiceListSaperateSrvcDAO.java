package com.dashboard.invoices.invoicelistsaperatesrvc;
...
public class ClsInvoiceListSaperateSrvcDAO {
 ...
 public JSONArray invoicelist(String branchval,String fromdate,String todate,String cldocno,String rentaltype,String agmtNo,String clstatuss,String cmbtariftype) throws SQLException {
	         ...
	         */
				...
				+ "if("+company+"=1,convert(coalesce(coalesce(sum(rent),0.0)+coalesce(sum(inschg),0.0)+coalesce(sum(accchg),0.0)+"+
				" coalesce(sum(salik), 0.0)+coalesce(sum(saliksrv),0.0)+coalesce(sum(traffic),0.0)+coalesce(sum(trafficsrv),0.0)+"+
				" coalesce(sum(others),0.0)+coalesce(sum(vat),0.0)+coalesce(sum(roundoff),0.0),0),char(50)),"+
				" convert(coalesce(coalesce(sum(rent),0.0)+coalesce(sum(inschg),0.0)+coalesce(sum(accchg),0.0)+ coalesce(sum(salik),0.0)+"+
				" coalesce(sum(saliksrv),0.0)+coalesce(sum(traffic),0.0)+coalesce(sum(trafficsrv),0.0)+coalesce(sum(others), 0.0)+"+
				" coalesce(sum(vat),0.0)+coalesce(sum(roundoff),0.0),0),char(50))) amount ,coalesce(sum(rent),0.0) rent,coalesce(sum(inschg),0.0) "+
				" inschg, coalesce(sum(accchg),0.0) accchg,coalesce(sum(salik),0.0) salikamt,coalesce(sum(saliksrv),0.0) saliksrvc,"+
				" coalesce(sum(traffic),0.0) trafficamt, coalesce(sum(trafficsrv),0.0) trafficsrvc,coalesce(sum(others),0.0) other, "+
				" coalesce(sum(vat),0.0) vat, round(coalesce(sum(roundoff),0.0),2) roundoff,"+
				...
				" on inv.brhid=br.doc_no left join my_salm ms on ms.doc_no=r.salid or ms.doc_no=l.salid where inv.status=3 and inv.date between '"+sqlfromdate+"' and '"+sqltodate+"' "+sqltest +" "+
