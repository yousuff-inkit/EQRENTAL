...
public class ClsLeaseAlmariahInvoiceAction {
	ClsConnection ClsConnection=new ClsConnection();

	ClsCommon ClsCommon=new ClsCommon();

	
ClsLeaseAlmariahInvoiceDAO leaseinvoiceDAO=new ClsLeaseAlmariahInvoiceDAO();
...
public String saveAction() throws ParseException, SQLException{
	...
	String mode=getMode();
	ClsLeaseAlmariahInvoiceBean bean=new ClsLeaseAlmariahInvoiceBean();
Connection conn = ClsConnection.getMyConnection();
try{

Statement stmt=conn.createStatement();
ClsLeaseAlmariahInvoiceDAO leaseinvDAO=new ClsLeaseAlmariahInvoiceDAO();
...
int fuelacno=0,securitypassacno=0,chaufferextraacno=0,hpdacno=0;
while(rsacno.next()){
	...
	chaufferextraacno=rsacno.getInt("chaufferextra");
...
//stmt.close();
	...
	if(mode.equalsIgnoreCase("A")){
		...
		for(int i=0;i<dataarray.length;i++){
			...
			for(int j=0;j <getInvgridlength();j++){
				...
				//System.out.println("INVLENGTH"+getInvgridlength());
				 System.out.println(invoicearray.get(j).trim());
				invoice=invoicearray.get(j).trim().split("::");
				invoice[0]=invoice[0].trim();
				invoice[1]=invoice[1].trim();
				invoice[2]=invoice[2].trim();
				invoice[3]=invoice[3].trim();
				invoice[4]=invoice[4].trim();
				invoice[5]=invoice[5].trim();
				invoice[6]=invoice[6].trim();
				invoice[7]=invoice[7].trim();
				invoice[8]=invoice[8].trim();
				invoice[9]=invoice[9].trim();
				invoice[10]=invoice[10].trim();
				invoice[11]=invoice[11].trim();
				invoice[12]=invoice[12].trim();
				invoice[13]=invoice[13].trim();
				invoice[14]=invoice[14].trim();
				invoice[15]=invoice[15].trim();
				invoice[16]=invoice[16].trim();
				invoice[17]=invoice[17].trim();
				invoice[18]=invoice[18].trim();
				invoice[19]=invoice[19].trim();
				invoice[20]=invoice[20].trim();
				invoice[21]=invoice[21].trim();
				invoice[22]=invoice[22].trim();
				invoice[23]=invoice[23].trim();
				invoice[24]=invoice[24].trim();
				invoice[25]=invoice[25].trim();
				invoice[26]=invoice[26].trim();
				...
				if(Double.parseDouble(invoice[22])>0){
					newarray.add("22"+"::"+securitypassacno+"::"+note+"::"+qty+"::"+invoice[22]+"::"+invoice[22]);
				}
				if(Double.parseDouble(invoice[23])>0){
					newarray.add("11"+"::"+fuelacno+"::"+note+"::"+qty+"::"+invoice[23]+"::"+invoice[23]);
				}
				if(Double.parseDouble(invoice[24])>0){
					newarray.add("2"+"::"+accacno+"::"+note+"::"+qty+"::"+invoice[24]+"::"+invoice[24]);
				}
				/*if(Double.parseDouble(invoice[25])>0){
					newarray.add("23"+"::"+diwacno+"::"+note+"::"+qty+"::"+invoice[25]+"::"+invoice[25]);
				}*/
				if(Double.parseDouble(invoice[26])>0){
					newarray.add("23"+"::"+chaufferextraacno+"::"+note+"::"+qty+"::"+invoice[26]+"::"+invoice[26]);
				}
				if(Double.parseDouble(invoice[25])>0){
					newarray.add("3"+"::"+chaufferacno+"::"+note+"::"+qty+"::"+invoice[25]+"::"+invoice[25]);
				}
	 ...
	 Date date1=new Date();
		...
		int val=leaseinvoiceDAO.customInvoiceInsert(conn,newarray, "LAG", sqldate,cldocno,agmtno,note,note, fromdate, todate, branchid,userid,
				currencyid, mode,invoice[4] , "INV###4", "INV",qty2);
					 if(val>0){
									 if(salikarray.size()>0){
										int valsalik=leaseinvoiceDAO.customInvoiceInsert(conn,salikarray, "LAG", sqldate,cldocno,agmtno,note,note, fromdate, todate, branchid,userid,
												currencyid, mode,invoice[4] , "INV###4", "INV",qty2);
										if(valsalik<=0){
											conn.close();
											return "fail";
									...
									if(trafficarray.size()>0){
										int valtraffic=leaseinvoiceDAO.customInvoiceInsert(conn,trafficarray, "LAG", sqldate,cldocno,agmtno,note,note, fromdate, todate, branchid,userid,
												currencyid, mode,invoice[4] , "INV###4", "INV",qty2);
										if(valtraffic<=0){
											conn.close();
											return "fail";
