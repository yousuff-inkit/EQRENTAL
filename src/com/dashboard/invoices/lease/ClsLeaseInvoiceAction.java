	...
	if(mode.equalsIgnoreCase("A")){
		...
		for(int i=0;i<dataarray.length;i++){
			...
			for(int j=0;j <getInvgridlength();j++){
				...
				/*rows[i].rano+" :: "+rows[i].ratype+" :: "+rows[i].fromdate+" :: "+rows[i].todate+" :: "+rows[i].acno+" :: "+rows[i].acname+" :: "+rows[i].amount+" 
				 * :: "+rows[i].cldocno+" :: "+rows[i].rentalsum+" :: "+rows[i].accsum+" :: "+rows[i].salikamt+" :: "+rows[i].trafficamt+" :: "+rows[i].saliksrvc+" 
				 * :: "+rows[i].trafficsrvc+" :: "+rows[i].datediff+" :: "+rows[i].brhid+" :: "+rows[i].curid+" :: "+rows[i].insurchg+" :: "+rows[i].salikcount+" 
				 * :: "+rows[i].trafficcount+" :: "+rows[i].salamount+" :: "+rows[i].salrate+"::"+rows[i].salikauhamt+"::"+rows[i].salikdxbamt+"::"+
				 * rows[i].salikauhsrvc+"::"+rows[i].salikdxbsrvc+"::"+rows[i].salikauhcount+"::"+rows[i].salikdxbcount+"::"+rows[i].salikauhrate+"::"+
				 * rows[i].salikdxbrate+"::"+rows[i].salikauhsrvcrate+"::"+rows[i].salikdxbsrvcrate*/
	 ...
	 Date date1=new Date();
		...
		int duplicatecount=manualdao.getDuplicateInvCount(conn, agmttype, agmtno+"", fromdate, todate, "4");
		int val=0;
		System.out.println("main duplicate "+duplicatecount);
		if(duplicatecount==0){
			val=manualdao.insert(conn,newarray, "LAG", sqldate,cldocno,agmtno,note,note, fromdate, todate, branchid,userid,
			currencyid, mode,invoice[4] , "INV###4", "INV",qty2);
		}
		if(val>0){
			if(salikarray.size()>0){
				duplicatecount=manualdao.getDuplicateInvCount(conn, agmttype, agmtno+"", fromdate, todate, "4");
				System.out.println("salik duplicate "+duplicatecount);
				int valsalik=0;
				if(duplicatecount<=1){
					manualdao.insert(conn,salikarray, "LAG", sqldate,cldocno,agmtno,note,note, fromdate, todate, branchid,userid,
					currencyid, mode,invoice[4] , "INV###4", "INV",qty2);
				}
			...
			if(trafficarray.size()>0){
				duplicatecount=manualdao.getDuplicateInvCount(conn, agmttype, agmtno+"", fromdate, todate, "4");
				System.out.println("traffic duplicate "+duplicatecount);
				int valtraffic=0;
				if(duplicatecount<=1){
					manualdao.insert(conn,trafficarray, "LAG", sqldate,cldocno,agmtno,note,note, fromdate, todate, branchid,userid,
					currencyid, mode,invoice[4] , "INV###4", "INV",qty2);
				}
				if(valtraffic<=0){
					conn.close();
