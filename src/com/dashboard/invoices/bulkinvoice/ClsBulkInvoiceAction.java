...
public class ClsBulkInvoiceAction {

	ClsCommon objcommon=new ClsCommon();
	ClsConnection objconn=new ClsConnection();
	ClsBulkInvoiceDAO objlease=new ClsBulkInvoiceDAO();
	...
	private String hidchkagmttype;
	
	
	
	public String getHidchkagmttype() {
		return hidchkagmttype;
	}
	public void setHidchkagmttype(String hidchkagmttype) {
		this.hidchkagmttype = hidchkagmttype;
	}
	...
	public String saveAction() throws ParseException, SQLException{
		...
		if(mode.equalsIgnoreCase("A")){
			...
			System.out.println("Agreement Type:"+getHidchkagmttype());
			int val=objlease.insert(getSelectedagmt(),getCmbtype(),getHidclient(),sqldate,getCmbbranch(),agmtarray,
					getPeriodupto(),session,request,getHidchkagmttype());
			setDetail("Invoices");
			setDetailname("Bulk Invoice");
			if(val>0){
