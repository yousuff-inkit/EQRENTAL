...
public class ClsAgeingStatementDAO  { 
	...
	public JSONArray ageingStatement(String branch,String uptodate,String atype,String accdocno, String salesperson,String category,String clientStatus,String level1from, String level1to, String level2from, String level2to,
			String level3from, String level3to,String level4from, String level4to,String level5from,String loadInBaseCurr,String check) throws SQLException {
		...
		try {
				...
				if(atype.equalsIgnoreCase("AR")){
					...
					select ="select a.*,cr.code currency,bk.period2 creditperiod,bk.credit creditlimit,bk.mail1 email,if(bk.per_mob is null,bk.com_mob,if(bk.per_mob='NA',bk.com_mob,if(bk.per_mob=' ',bk.com_mob,bk.per_mob))) mobile_no,bk.contactPerson contact_person,s.sal_name from (select name 'account_name',CONVERT(if(sum(t7+u6)<0,round((sum(t7+u6)*-1),2),''),CHAR(50)) 'advance',\r\n" + 
							"CONVERT(if(sum(t7+u6)>0,round((sum(t7+u6)),2),''),CHAR(50)) 'balance',CONVERT(if(sum(u6<0),round((sum(u6*-1)),2),''),CHAR(50)) 'unapplied',\r\n" + 
							"CONVERT(if(sum(t7)>0,round((sum(t7)),2),''),CHAR(50)) 'total',CONVERT(if(sum(l1)>0,round((sum(l1)),2),''),CHAR(50)) 'level_1',\r\n" + 
							"CONVERT(if(sum(l2)>0,round((sum(l2)),2),''),CHAR(50)) 'level_2',CONVERT(if(sum(l3)>0,round((sum(l3)),2),''),CHAR(50)) 'level_3',\r\n" + 
					...
					select ="select a.*,cr.code currency,bk.period2 creditperiod,bk.credit creditlimit,bk.mail1 email,if(bk.per_mob is null,bk.com_mob,if(bk.per_mob='NA',bk.com_mob,if(bk.per_mob=' ',bk.com_mob,bk.per_mob))) mobile_no,bk.contactPerson contact_person,s.sal_name from (select name 'account_name',CONVERT(if(sum(t7+u6)>0,round((sum(t7+u6)),2),''),CHAR(50)) 'advance',\r\n" + 
							"CONVERT(if(sum(t7+u6)<0,round((sum(t7+u6)*-1),2),''),CHAR(50)) 'balance',CONVERT(if(sum(u6>0),round((sum(u6)),2),''),CHAR(50)) 'unapplied',\r\n" + 
							"CONVERT(if(sum(t7)<0,round((sum(t7)*-1),2),''),CHAR(50)) 'total',CONVERT(if(sum(l1)<0,round((sum(l1)*-1),2),''),CHAR(50)) 'level_1',\r\n" + 
							"CONVERT(if(sum(l2)<0,round((sum(l2)*-1),2),''),CHAR(50)) 'level_2',CONVERT(if(sum(l3)<0,round((sum(l3)*-1),2),''),CHAR(50)) 'level_3',\r\n" + 
				...
				/*sql = select+" (select j.acno,j.brhid,h.description name,bk.com_mob mob,bk.per_mob pmob,bk.contactPerson contact,sum(dramount) - coalesce(o.amount,0)*id bal, j.tranid, j.doc_no,"
				 ...
				 if((loadInBaseCurr.equalsIgnoreCase("1"))){
					  sql = select+" (select j.acno,j.brhid,h.description name,(sum(dramount) - coalesce(o.amount,0)*id)*j.rate bal, j.tranid, j.doc_no,TIMESTAMPDIFF(Day,cast(j.date as datetime),cast('"+sqlUpToDate+"' as datetime)) duedys\r\n" + 
						...
						" union all select j.acno,j.brhid,h.description name,(sum(dramount)- coalesce(o.amount,0)*id)*j.rate bal, j.tranid, j.doc_no,TIMESTAMPDIFF(Day,cast(j.date as datetime),cast('"+sqlUpToDate+"' as datetime)) duedys\r\n" + 
						...
						" and bk.status=3 left join my_salm s on bk.sal_id=s.doc_no left join my_curr cr on cr.doc_no=bk.curId where 1=1 "+sql1;
