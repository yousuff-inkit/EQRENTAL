	...
	public String saveAction() throws ParseException, SQLException{
		...
		if(mode.equalsIgnoreCase("A")){
			...
			if(val>0.0){
				...
				/*String sql="select coalesce(postedtype,'') postedtype,coalesce(postedno,'') postedno from gl_bcpo where doc_no='"+val+"'";
				System.out.println("sql===="+sql); 
				  ResultSet resultSet = stmt.executeQuery(sql); 
				    if (resultSet.next()) {
				    	dtype=resultSet.getString("postedtype");   
				    	postno=resultSet.getString("postedno");  
				    }
				setMsg(dtype+"-"+postno+"  Successfully Generated");*/
