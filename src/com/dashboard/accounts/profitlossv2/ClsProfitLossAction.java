...
public class ClsProfitLossAction extends ActionSupport{
	...
	public String printAction() throws ParseException, SQLException{
		...
		String branchval = request.getParameter("branchval")==null || request.getParameter("branchval").trim().equals("") || request.getParameter("branchval").trim().equals("a")?"1":request.getParameter("branchval").trim().toString();
		...
		System.out.println("Brach add on Action : "+profitlossBean.getLblcompaddress());
		...
		System.out.print("Branch is :"+profitlossBean.getLblbranch());
			...
			HttpServletResponse response = ServletActionContext.getResponse();
			 ...
			 try {
				 param = new HashMap();
             	 ...
             	 String imgheader="",imgfooter="";   
             	 String brchsql = "SELECT imgpath, imgfooter FROM my_brch WHERE doc_no='"+branchval+"'";
             	 //System.out.println("brchsql=="+brchsql);
             	 ResultSet rs1 = profitloss.executeQuery(brchsql);
             	 while(rs1.next()) {
             		imgheader = rs1.getString("imgpath");
             		imgfooter = rs1.getString("imgfooter");   
             	 }
            	 String imgheaderseal=request.getSession().getServletContext().getRealPath(imgheader);
               	 if(imgheaderseal!=null && !imgheaderseal.equals("")) {   
               	  imgheaderseal=imgheaderseal.replace("\\", "\\\\");
               	 }
               	 String imgfooterseal=request.getSession().getServletContext().getRealPath(imgfooter);
               	 if(imgfooterseal!=null && !imgfooterseal.equals("")) {   
               	  imgfooterseal=imgfooterseal.replace("\\", "\\\\");  
                 }
               	 //System.out.println(imgheader+"=PRINT IMG PATH="+imgfooter);    
               	 param.put("imgheaderseal", imgheaderseal);
               	 param.put("imgfooterseal", imgfooterseal);  
		         ...
		         param.put("printname", profitlossBean.getLblprintname()); 
		         System.out.println("LBLBranch addrss : "+getLblbranchaddress());
