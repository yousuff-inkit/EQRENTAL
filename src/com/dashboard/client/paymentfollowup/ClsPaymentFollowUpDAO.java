...
public class ClsPaymentFollowUpDAO  {
	...
	public JSONArray paymentFollowUpGridLoading(String branch,String uptodate,String clientaccount,String chkfollowup,String followupdate,String salesperson,String category,String amtrangefrm,String amtrangeto,String clientStatus,String check) throws SQLException {
		...
		try {
				...
				if(!(clientStatus.equalsIgnoreCase(""))){
					...
					else if(clientStatus.equalsIgnoreCase("8")){
						sql3+=" and bk.pcase=4";
					}
	    		}
					
				sql = "select CONVERT(coalesce(bv.fdate,''),CHAR(100)) fdate,a.*,cr.code currency,bk.cldocno,bk.contactPerson 'contact',if(bk.per_mob is null,bk.com_mob,bk.per_mob) pmob,bk.mail1 email,s.sal_name from ( select ag.name,ag.acno,ag.brhid,ag.doc_no,CONVERT(if(sum(t7+u6)<0,round((sum(t7+u6)*-1),2),''),CHAR(50)) advance,\r\n" + 
					...
					" left join my_acbook bk on a.acno=bk.acno and bk.status=3 left join my_curr cr on cr.doc_no=bk.curId left join my_salm s on bk.sal_id=s.doc_no left join (select max(doc_no) doc_no,rdocno from gl_bcpf where status=3 group by rdocno) sub on(sub.rdocno=bk.cldocno) left join gl_bcpf bv on sub.doc_no = bv.doc_no  where 1=1"+sql2+""+sql3+" order by bk.cldocno";
		...
		try {
				...
				*/
				...
				sql = "select a.name 'Account Name',bk.contactPerson 'Contact Person',if(bk.per_mob is null,bk.com_mob,bk.per_mob) 'Mobile No',a.advance 'Advance',a.balance 'Balance',a.unapplied 'Unapplied',a.netamount 'Total',a.level1 'Level 1[0-30]',a.level2 'Level 2[31-60]',a.level3 'Level 3[61-90]',\r\n" +
					...
					" round((d.bal),2),0) l4,if(d.duedys >=121 and d.bal>0,round((d.bal),2),0) l5,CONVERT(if(d.bal<0,round((d.bal),2),''),CHAR(50)) U6,if(d.bal>0,d.bal,0) t7 from (select j.acno,j.brhid,h.description name,sum(dramount-out_amount) bal,j.tranid, j.doc_no,\n\r" +
					" TIMESTAMPDIFF(Day,cast(j.date as datetime),cast('"+sqlUpToDate+"' as datetime)) duedys from my_jvtran j inner join my_head h on j.acno=h.doc_no where j.status=3 and h.atype='AR' and j.yrid=0 and j.date<='"+sqlUpToDate+"' "+sql+" group by tranid having bal<>0) d) ag group by acno"+sql1+") a\r\n" + 
					" left join my_acbook bk on a.acno=bk.acno and bk.status=3 left join my_salm s on bk.sal_id=s.doc_no left join (select max(doc_no) doc_no,rdocno from gl_bcpf where status=3 group by rdocno) sub on(sub.rdocno=bk.doc_no) left join gl_bcpf bv on sub.doc_no = bv.doc_no where 1=1"+sql2+""+sql3+" order by bk.cldocno";
	...
	public JSONArray paymentFollowUpDetailGrid(String cldocno,String check) throws SQLException {
		...
		try {
				...
				String sql = "select m.date detdate,m.remarks remk,m.fdate,u.user_id user from gl_bcpf m inner join my_user u on u.doc_no=m.userid where m.rdocno="+cldocno+" "
						+ "and m.bibpid=(select rowno from gl_bibp where bibdid=(select doc_no from gl_bibd where description='Payment Followup') and process='Follow-Up') and m.status=3 group by m.doc_no order by m.fdate desc";
	...
	public  ClsPaymentFollowUpBean getRAWiseBalancePrint(HttpServletRequest request,String atype,int acno,String branch,String uptodate) throws SQLException {
	...
	try {
		...
		sql="select 'RA Wise Balance' vouchername,(DATE_FORMAT('"+sqlUpToDate+"','%D %M  %Y ')) vouchername1,bk.address accountaddress,bk.per_mob accountmob,"
			+ "coalesce(bk.period,0) minperiod,coalesce(bk.period2,0) maxperiod,coalesce(bk.credit,0) creditlimit,j.acno,t.account,t.description,c.company,c.address,"
			+ "c.tel,c.fax,b.branchname,b.pbno,b.stcno,b.cstno,l.loc_name location,cd.code from my_acbook bk left join my_jvtran j on  bk.acno=j.acno  left join my_head t "
		    + "on j.acno=t.doc_no left join my_brch b on j.brhid=b.doc_no left join my_locm l on l.brhid=b.doc_no left join my_comp c on b.cmpid=c.doc_no left join my_curr cd "
			+ "on cd.doc_no=j.curId where j.acno="+acno+" and j.yrid=0 and j.status=3 group by acno";
	...
	public JSONArray ageingStatement(String branch,String uptodate,String accdocno,String chkfollowup,String followupdate,String salesperson,String category,String amtrangefrm,String amtrangeto,String clientStatus,String check) throws SQLException {
		JSONArray RESULTDATA=new JSONArray();  
		if(!check.equalsIgnoreCase("1")) {
			return RESULTDATA;
		}
        ...
        java.sql.Date sqlUpToDate = null;
        java.sql.Date sqlFollowUpDate = null; 
        String level1from="0",  level1to="30",  level2from="31",  level2to="60", level3from="61",  level3to="90", level4from="91",  level4to="120", level5from="121";
		try {
				conn = ClsConnection.getMyConnection();
				Statement stmtAgeingStatement = conn.createStatement();
				String sql = "",sql1="",sqld = "",sqld1 = "",select="",atype="AR",sql2="",sql3="",sql4="";   
				...
				if(atype.equalsIgnoreCase("AR")){
					...
					select ="select a.*,cr.code currency,if(bk.per_mob is null,bk.com_mob,if(bk.per_mob='NA',bk.com_mob,if(bk.per_mob=' ',bk.com_mob,bk.per_mob))) pmob,bk.mail1 email,bk.cldocno,bk.contactPerson contact,s.sal_name from ("
						 ...
						 + " CONVERT((if(sum(t7+u6)>0,round((sum(t7+u6)),2)-(if(sum(l0)>0,round((sum(l0)),2),0)),'')),CHAR(50)) 'duebalance',"
						 ...
						 + "CONVERT(if(sum(l4)>0,round((sum(l4)),2),''),CHAR(50)) 'level4',CONVERT(if(sum(l5)>0,round((sum(l5)),2),''),CHAR(50)) 'level5', ag.doc_no, ag.acno,ag.brhid from (select d.name,d.acno,d.brhid,"
					...
					select ="select a.*,cr.code currency,if(bk.per_mob is null,bk.com_mob,if(bk.per_mob='NA',bk.com_mob,if(bk.per_mob=' ',bk.com_mob,bk.per_mob))) pmob,bk.mail1 email,bk.cldocno,bk.contactPerson contact,s.sal_name from ("
						   ...
						   + "CONVERT(if(sum(l4)<0,round((sum(l4)*-1),2),''),CHAR(50)) 'level4',CONVERT(if(sum(l5)<0,round((sum(l5)*-1),2),''),CHAR(50)) 'level5', ag.doc_no,ag.acno,ag.brhid from (select d.name,d.acno,"
				...
				if(chkfollowup.equalsIgnoreCase("1")){
					if(!(sqlFollowUpDate==null)){
			        	sql1+=" and bv.fdate<='"+sqlFollowUpDate+"'";
					}
				}
				
				if(!(((amtrangefrm.equalsIgnoreCase("")) && (amtrangeto.equalsIgnoreCase(""))) || ((amtrangefrm.equalsIgnoreCase("0")) && (amtrangeto.equalsIgnoreCase("0"))) )){
	    			sql4 = " having balance between "+amtrangefrm+" and "+amtrangeto+"";
	    		}
				
				if(!(clientStatus.equalsIgnoreCase(""))){
					if(clientStatus.equalsIgnoreCase("1")){
						sql1+=" and (bk.rostatus!=0 or bk.lostatus!=0)  and bk.pcase=0";
					}
					else if(clientStatus.equalsIgnoreCase("2")){
						sql1+=" and (bk.rostatus=0 and bk.lostatus=0)  and bk.pcase=0";
					}
					else if(clientStatus.equalsIgnoreCase("3")){
						sql1+=" and (bk.rostatus!=0 or bk.lostatus!=0) and bk.pcase=1";
					}
					else if(clientStatus.equalsIgnoreCase("4")){
						sql1+="  and (bk.rostatus=0 and bk.lostatus=0) and bk.pcase=1";
					}
					else if(clientStatus.equalsIgnoreCase("5")){
						sql1+=" and (bk.rostatus!=0 or bk.lostatus!=0) and bk.pcase=2";
					}
					else if(clientStatus.equalsIgnoreCase("6")){
						sql1+="  and (bk.rostatus=0 and bk.lostatus=0) and bk.pcase=2";
					}
					else if(clientStatus.equalsIgnoreCase("7")){
						sql1+=" and bk.pcase=3";
					}
				...
				sql = select+" (select j.acno,j.brhid,h.description name,sum(dramount) - coalesce(o.amount,0)*id bal, j.tranid, j.doc_no,TIMESTAMPDIFF(Day,cast(coalesce(j.duedate,j.date) as datetime),cast('"+sqlUpToDate+"' as datetime)) duedys " 
					...
					+ " group by tranid) o on j.tranid=o.tranid where j.status=3 and h.atype='"+atype+"' and j.date<='"+sqlUpToDate+"'"+sql+""+sqld+" group by j.tranid having bal<>0) d) ag group by acno"+sql4+") a  left join my_acbook bk on a.acno=bk.acno " 
					+ " and bk.status=3 left join my_curr cr on cr.doc_no=bk.curId left join my_salm s on bk.sal_id=s.doc_no left join (select max(doc_no) doc_no,rdocno from gl_bcpf where status=3 group by rdocno) sub on(sub.rdocno=bk.cldocno) left join gl_bcpf bv on sub.doc_no = bv.doc_no where 1=1 "+sql1+"";  
