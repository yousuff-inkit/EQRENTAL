					...
					+ " union all select br.branchname,date_format(m.date,'%d-%m-%Y') date,h.description,''area,0 trnno,m.dtype,convert(m.doc_no,char(100)) vocno,m.refno,date_format(m.date,'%d-%m-%Y') date,"    
  			        ...
  			        + " left join my_head h on h.doc_no=m.acno left join my_brch br on m.brhid=br.doc_no where m.status=3 and m.dtype='TCN'  and h.atype in ('GL','AP') and  m.date between '"+sqlfromdate+"' and '"+sqltodate+"' "+sqltest+"  group by m.tr_no "	
                       + "  union all"
  						...
  						+ " sum(if(d.nettotal<0,d.nettotal*-1,d.nettotal))*-1 totalvalue,sum(if(taxamount!=0,if(d.amount<0,d.amount*-1,d.amount),0))*-1 vatapplied,sum(if(taxamount=0,if(d.amount<0,d.amount*-1,d.amount),0))*-1  vatnotapplied, sum(coalesce(if(d.taxamount<0,d.taxamount*-1,d.taxamount),0))*-1  vatcollected,'' type,'' exempted  from my_cnot m left join my_cnotd d on" 
...
//			System.out.println("==== "+branch);
			...
			strsql=" select a.* from(select B.branchname,date_format(m.date,'%d-%m-%Y') date,A.refname customer_name,coalesce(ar.area,'')area,convert(A.trnnumber,char(100)) trnno,m.dtype doctype,convert(m.voc_no,char(100)) docno , "
					...
					+ " sum(if(d.nettotal<0,d.nettotal,d.nettotal)) totalvalue,sum(if(taxamount!=0,if(d.amount<0,d.amount,d.amount),0))*-1 vatapplied,sum(if(taxamount=0,if(d.amount<0,d.amount,d.amount),0))*-1  vatnotapplied, sum(coalesce(if(d.taxamount<0,d.taxamount,d.taxamount),0))*-1  vatcollected,'' type,'' exempted from my_cnot m left join my_cnotd d on" 
					+ " m.tr_no=d.tr_no left join my_head h on h.doc_no=m.acno "
