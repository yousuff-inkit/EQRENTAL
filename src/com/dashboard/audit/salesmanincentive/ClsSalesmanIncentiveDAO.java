package com.dashboard.audit.salesmanincentive;
...
public class ClsSalesmanIncentiveDAO  {
	
	ClsConnection connDAO=new ClsConnection();
	ClsCommon commonDAO=new ClsCommon();

  public JSONArray GridLoading(String branch, String fromdate, String todate, String acno, String check) throws SQLException {
	...
	Connection conn = null;  
	  ...
	  try {
		    ...
		    if(check.equalsIgnoreCase("1")) {
		        ...
		        sql = " select t.date,t.dtype,t.doc_no,q.chqno,q.chqdt,@xcr:=(round(if(t.dramount>0,t.dramount*t.id,0),2)) cr, "
		        		+ " @xdr:=(round(if(t.dramount<0,t.dramount*t.id,0),2)) dr,if(t.bankreconcile>0,1,0) chk,t.clearedon c_date, "
		        		+ " t.description,t.ref_detail,t.tranid, maxtrid,pj.acno ac,a.refname party,sm.sal_name  salesman "
		        		+ "from my_jvtran t inner join my_brch b on t.brhId=b.doc_no "
		        		+ "left join my_head h on h.doc_no=t.acno "
		        		+ "left join (select max(tranid) maxtrid,tr_no from my_jvtran group by tr_no) mt on mt.tr_no=t.tr_no "
		        		+ "left join my_jvtran pj on pj.tranid=mt.maxtrid "
		        		+ "left join my_acbook a on a.acno=pj.acno "
		        		...
		        		+ " where  t.status=3 and t.yrid=0  and h.den=305 "+sql+" ";
