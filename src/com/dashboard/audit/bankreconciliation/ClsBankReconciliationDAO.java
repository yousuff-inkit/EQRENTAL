	  ...
	  try {
		    ...
		    if(check.equalsIgnoreCase("1")) {
		        ...
		        sql = "select t.date,t.dtype,t.doc_no,q.chqno,q.chqdt,@xcr:=(round(if(t.dramount>0,t.dramount*t.id,0),2)) cr,@xdr:=(round(if(t.dramount<0,t.dramount*t.id,0),2)) dr,"
		        		+ "if(t.bankreconcile>0,1,0) chk,t.clearedon c_date,t.description,t.ref_detail,t.tranid,(select max(tranid) from my_jvtran where doc_no=t.doc_no and dtype=t.dtype  and brhid=t.brhid ) maxtrid,"
		        		+ "(select acno from my_jvtran where doc_no=t.doc_no and dtype=t.dtype  and brhid=t.brhid and tranid=maxtrid) ac,(select description from my_head where doc_no=ac) party from my_jvtran t "
