package com.dashboard.rentalagreementlatest;
...
*/            		ResultSet resultSet = stmtVeh.executeQuery(sql);
	...
	public JSONArray rtaupdate(String brnchval,String type) throws SQLException {
		...
		try {
            	...
            	{   if(type.equalsIgnoreCase("rental"))
            	     {
            		String sql="select r.rtaremarks upd,'Confirm' btnconfirm,r.voc_no,'Attach' btnattach,'Edit' btnsave,r.brhid, r.doc_no,r.date,r.fleet_no,r.cldocno,r.odate,r.otime,r.okm,CASE WHEN r.ofuel='0.000' THEN 'Level 0/8' WHEN r.ofuel='0.125' THEN 'Level 1/8' WHEN r.ofuel='0.250' THEN 'Level 2/8' WHEN r.ofuel='0.375' "
            				+ "THEN 'Level 3/8'  WHEN r.ofuel='0.500' THEN 'Level 4/8' WHEN r.ofuel='0.625' THEN 'Level 5/8' "
            				+ "WHEN r.ofuel='0.750' THEN 'Level 6/8' WHEN r.ofuel='0.875' THEN 'Level 7/8' WHEN r.ofuel='1.000' THEN 'Level 8/8' "
            				+ "END as 'ofuel',r.ofuel hidfuel,r.drid,v.reg_no,v.flname,a.refname from gl_ragmt r "
            				+ "left join gl_vehmaster v on r.fleet_no=v.fleet_no left join my_acbook a on a.cldocno=r.cldocno and a.dtype='CRM'   where  r.clstatus=0  and r.rtaconfirm=0  ";
            	System.out.println("rental a----"+sql);
            		ResultSet resultSet = stmtVeh.executeQuery(sql);
            		 RESULTDATA=ClsCommon.convertToJSON(resultSet);
     				stmtVeh.close();
            	     }
       	     ...
       	     {
            		String sql="select r.rtaremarks upd,'Confirm' btnconfirm,r.voc_no, 'Attach' btnattach,'Edit' btnsave,r.brhid, r.doc_no,r.date,if(perfleet=0,tmpfleet, perfleet) fleet_no,r.cldocno,r.outdate "
            				...
            				+ "  where  r.status=3 and r.clstatus=0  and r.vehupdate=1 and r.rtaconfirm=0 ";
            	System.out.println("lease a======"+sql);
            		ResultSet resultSet = stmtVeh.executeQuery(sql);
            		 RESULTDATA=ClsCommon.convertToJSON(resultSet);
     				stmtVeh.close();
            		...
            		String sql="select  r.rtaremarks upd,'Confirm' btnconfirm,r.voc_no,'Attach' btnattach,'Edit' btnsave,r.brhid, r.doc_no,r.date,r.fleet_no,r.cldocno,r.odate,r.otime,r.okm,CASE WHEN r.ofuel='0.000' THEN 'Level 0/8' WHEN r.ofuel='0.125' THEN 'Level 1/8' WHEN r.ofuel='0.250' THEN 'Level 2/8' WHEN r.ofuel='0.375' "
            				...
            				+ "END as 'ofuel',r.ofuel hidfuel,r.drid,v.reg_no,v.flname,a.refname from gl_ragmt r "
            				+ "left join gl_vehmaster v on r.fleet_no=v.fleet_no left join my_acbook a on a.cldocno=r.cldocno and a.dtype='CRM'  "
            				+ "  where  r.clstatus=0  and r.rtaconfirm=0  and  r.brhid='"+brnchval+"' ";
            	 	System.out.println("else rental----"+sql);
            		ResultSet resultSet = stmtVeh.executeQuery(sql);
            	 RESULTDATA=ClsCommon.convertToJSON(resultSet);
 				stmtVeh.close();
            	     }
            		 if(type.equalsIgnoreCase("lease"))
            	     {
            			 ...
            			 String sql="select r.rtaremarks upd,'Confirm' btnconfirm,r.voc_no,'Attach' btnattach,'Edit' btnsave,r.brhid, r.doc_no,r.date,if(perfleet=0,tmpfleet, perfleet) fleet_no,r.cldocno,r.outdate "
                 				...
                 				+ "gl_lagmt r left join gl_vehmaster v on  v.fleet_no=r.perfleet "
                 				+ "left join gl_vehmaster v1 on  v1.fleet_no=r.tmpfleet  left join my_acbook a on a.cldocno=r.cldocno and a.dtype='CRM'  "
                 				+ "  where  r.status=3 and r.clstatus=0  and  r.vehupdate=1 and r.rtaconfirm=0 and r.brhid='"+brnchval+"' ";
                 	
                 		System.out.println("else lease======"+sql);
	...
	public JSONArray rtaupdateExcel(String brnchval,String type) throws SQLException {
		...
		try {
            	...
            	{   if(type.equalsIgnoreCase("rental"))
            	     {
            		String sql="select r.voc_no 'RA NO',a.refname 'CLIENT NAME',r.fleet_no 'FLEET NO',v.reg_no 'REG NO',r.odate 'OUT DATE',r.otime 'TIME',r.okm 'KM',CASE WHEN r.ofuel='0.000' THEN 'Level 0/8' WHEN r.ofuel='0.125' THEN 'Level 1/8' WHEN r.ofuel='0.250' THEN 'Level 2/8' WHEN r.ofuel='0.375' "
            				+ "THEN 'Level 3/8'  WHEN r.ofuel='0.500' THEN 'Level 4/8' WHEN r.ofuel='0.625' THEN 'Level 5/8' "
            				+ "WHEN r.ofuel='0.750' THEN 'Level 6/8' WHEN r.ofuel='0.875' THEN 'Level 7/8' WHEN r.ofuel='1.000' THEN 'Level 8/8' "
            				+ "END as 'FUEL',r.rtaremarks 'RTA REMARKS' from gl_ragmt r "
            				+ "left join gl_vehmaster v on r.fleet_no=v.fleet_no left join my_acbook a on a.cldocno=r.cldocno and a.dtype='CRM'   where  r.clstatus=0  and r.rtaconfirm=0  ";
            	System.out.println("rentalEX a----"+sql);
            		ResultSet resultSet = stmtVeh.executeQuery(sql);
            	...
            	if(type.equalsIgnoreCase("lease"))
       	     {
            		String sql="select r.voc_no 'RA NO',a.refname 'CLIENT NAME',if(perfleet=0,tmpfleet, perfleet) 'FLEET NO',coalesce(v.reg_no,v1.reg_no) 'REG NO',r.outdate 'OUT DATE',r.outtime 'TIME',r.outkm 'KM', "
            				        + " CASE WHEN r.outfuel='0.000' THEN 'Level 0/8' WHEN r.outfuel='0.125' THEN 'Level 1/8' "
                    				+ " WHEN r.outfuel='0.250' THEN 'Level 2/8' WHEN r.outfuel='0.375'	THEN 'Level 3/8' "
                    				+ "	WHEN r.outfuel='0.500' THEN 'Level 4/8'  WHEN r.outfuel='0.625' THEN 'Level 5/8' "
                    				+ " WHEN r.outfuel='0.750' THEN 'Level 6/8' WHEN  r.outfuel='0.875'  THEN 'Level 7/8' WHEN r.outfuel='1.000' THEN 'Level 8/8' "
                    				+ " END as 'FUEL',r.rtaremarks 'RTA REMARKS' from gl_lagmt r left join gl_vehmaster v on  v.fleet_no=r.perfleet "
            				+ "left join gl_vehmaster v1 on  v1.fleet_no=r.tmpfleet  left join my_acbook a on a.cldocno=r.cldocno and a.dtype='CRM'  "
            				+ "  where  r.status=3 and r.clstatus=0  and r.vehupdate=1 and r.rtaconfirm=0 ";
            	System.out.println("leaseEX a======"+sql);
            		ResultSet resultSet = stmtVeh.executeQuery(sql);
            		 RESULTDATA=ClsCommon.convertToEXCEL(resultSet);
     				stmtVeh.close();
            		...
            		String sql="select r.voc_no 'RA NO',a.refname 'CLIENT NAME',r.fleet_no 'FLEET NO',v.reg_no 'REG NO',r.odate 'OUT DATE',r.otime 'TIME',r.okm 'KM',CASE WHEN r.ofuel='0.000' THEN 'Level 0/8' WHEN r.ofuel='0.125' THEN 'Level 1/8' WHEN r.ofuel='0.250' THEN 'Level 2/8' WHEN r.ofuel='0.375' "
            				...
            				+ "left join gl_vehmaster v on r.fleet_no=v.fleet_no left join my_acbook a on a.cldocno=r.cldocno and a.dtype='CRM'  "
            				+ "  where  r.clstatus=0  and r.rtaconfirm=0  and  r.brhid='"+brnchval+"' ";
            	 	System.out.println("else rentalEX----"+sql);
            		ResultSet resultSet = stmtVeh.executeQuery(sql);
            	 RESULTDATA=ClsCommon.convertToEXCEL(resultSet);
 				stmtVeh.close();
            	     }
            		 if(type.equalsIgnoreCase("lease"))
            	     {
                 		...
                 		System.out.println("else leaseEX======"+sql);
		...
		try {
            	...
            	if(brnchval.equalsIgnoreCase("a"))
            	{
            		...
            		String sql ="select r.voc_no,bv.fdate,r.brhid, r.doc_no,r.fleet_no,r.cldocno,r.odate,r.otime,r.okm,r.ddate,r.dtime,  ms.sal_name ,"
            				...
            				+ " j.rtype='rag'  group by rdocno) jv on jv.docno=r.doc_no    where   r.clstatus=0 and r.dispute=0 and r.ddate< (select curdate()+10)  order by ddate ";
        System.out.println(sql);
            		ResultSet resultSet = stmtVeh.executeQuery(sql);
            		 RESULTDATA=ClsCommon.convertToJSON(resultSet);
     				stmtVeh.close();
            		...
            		String sql ="select r.voc_no,bv.fdate,r.brhid, r.doc_no,r.fleet_no,r.cldocno,r.odate,r.otime,r.okm,r.ddate,r.dtime,  ms.sal_name ,"
            				...
            				+ "gl_bvdd bv on b.doc_no=bv.doc_no    left join my_salm ms on ms.doc_no=r.salid left join gl_rtarif rt on r.doc_no=rt.rdocno and rt.rstatus=7 left join"
            				+ " ( select l.doc_no docno,sum(coalesce(j.dramount,0)) sum from gl_ragmt l inner join my_jvtran j  on j.acno=l.acno and l.doc_no=j.rdocno and"
            				+ " j.rtype='rag'  group by rdocno) jv on jv.docno=r.doc_no     where   r.clstatus=0 and r.dispute=0 and r.ddate< (select curdate()+10) and r.brhid='"+brnchval+"' ";
            		System.out.println("========"+sql);
						...
						if(brnchval.equalsIgnoreCase("a"))
		            	{
		            		...
		            		String sql ="select r.voc_no 'RA No',r.fleet_no 'Fleet',v.FLNAME 'Fleet Name',v.reg_no 'Reg No',a.refname 'Client Name',a.contactperson 'Contact Person',a.per_mob 'Mob No',t.rentaltype \r\n" + 
		        		   	"'Rental type',r.odate 'Out Date',r.otime 'Out time',r.ddate 'Due Date',r.dtime 'Due Time',bv.fdate 'Followup Date', ms.sal_name 'Sales Man',coalesce(rt.rate,0)Rate,coalesce(jv.sum,0) Credit_Balance from gl_ragmt r "
		            				...
		            				+ "gl_bvdd bv on b.doc_no=bv.doc_no  left join my_salm ms on ms.doc_no=r.salid left join gl_rtarif rt on r.doc_no=rt.rdocno and rt.rstatus=7 left join"
            				        + " ( select l.doc_no docno,sum(coalesce(j.dramount,0)) sum from gl_ragmt l inner join my_jvtran j  on j.acno=l.acno and l.doc_no=j.rdocno and"
            				        + " j.rtype='rag'  group by rdocno) jv on jv.docno=r.doc_no  where   r.clstatus=0 and r.dispute=0 and r.ddate< (select curdate()+10)   ";
