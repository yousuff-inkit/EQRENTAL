...
public class ClsServiceSaleListDAO {
	...
	public JSONArray loadSummaryData(String fromdate, String todate, String clientid, String brch, String chk)
			throws SQLException {
		...
		try {
			...
			String sqldata = "select sm.doc_no as salid,sm.sal_name as salname,m.rate,c.code currency,m.brhid,m.doc_no,m.voc_no,m.date,a.refname client,coalesce(m.invno,'') invno,m.desc1,sum(d.nettaxamount) total from my_srvsalem m "
					+ "left join my_head h on h.doc_no=m.acno left join my_acbook a on a.acno=h.doc_no and a.dtype='CRM' "
					+ "left join my_srvsaled d on m.doc_no=d.rdocno left join my_salm sm on(a.sal_id=sm.doc_no) left join my_curr c on c.doc_no=m.curid where m.status<>7 " + sqltest1 + " " + sqltest
					+ " and m.date>='" + sqlfromdate + "' and m.date<='" + sqltodate + "' group by d.rdocno ";
          // System.out.println("summary grid-------------"+sqldata);
	...
	public JSONArray loadSummaryExcel(String fromdate, String todate, String clientid, String brch, String chk)
			throws SQLException {
		...
		try {
			...
			String sqldata = "select m.voc_no 'DOCNO',m.date 'DATE',m.brhid 'BRANCH',coalesce(m.invno,'')'PO NO',a.refname 'CLIENT',m.desc1 'DESCRIPTION',sum(d.nettaxamount)'TOTAL' from my_srvsalem m "
					+ "left join my_head h on h.doc_no=m.acno left join my_acbook a on a.acno=h.doc_no and a.dtype='CRM' "
					+ "left join my_srvsaled d on m.doc_no=d.rdocno where m.status<>7  " + sqltest1 + " " + sqltest
					+ " and m.date>='" + sqlfromdate + "' and m.date<='" + sqltodate + "' group by d.rdocno ";
	...
	public JSONArray loadDetailData(String fromdate, String todate, String clientid, String brch, String chk)
			throws SQLException {
		...
		try {
			...
			String sqldata = "select m.rate,c.code currency,sm.doc_no as salid,sm.sal_name as salname,m.brhid,m.voc_no,a.refname client,d.srno,d.desc1 description,d.unitprice,d.qty,d.qty qutval,d.total,d.discount,d.nettotal,d.nuprice,"
					...
					+ "left join my_costunit u on u.costtype=d.costtype left join my_acbook a on a.acno=hd.doc_no and a.dtype='CRM' left join my_salm sm on(a.sal_id=sm.doc_no) left join my_curr c on c.doc_no=m.curid where m.status<>7 and m.date>='"
					+ sqlfromdate + "' and m.date<='" + sqltodate + "' " + sqltest + " " + sqltest1 + "";

			//System.out.println("detail grid------------" + sqldata);
			resultSet = stmt.executeQuery(sqldata);
			RESULTDATA = cm.convertToJSON(resultSet);
			// System.out.println("=====RESULTDATA"+RESULTDATA);
	...
	public JSONArray loadDetailExcel(String fromdate, String todate, String clientid, String brch, String chk)
			throws SQLException {
		...
		try {
			...
			String sqldata = " select m.voc_no 'DOCNO',m.brhid 'BRANCH',a.refname 'CLIENT',d.desc1 'DESCRIPTION',d.qty 'QTY',d.unitprice 'UNIT PRICE',d.total 'TOTAL',d.discount 'DISCOUNT',d.nettotal 'NETTOTAL',"
					...
					+ "left join my_costunit u on u.costtype=d.costtype left join my_acbook a on a.acno=hd.doc_no and a.dtype='CRM' where m.status<>7 and m.date>='"
					+ sqlfromdate + "' and m.date<='" + sqltodate + "' " + sqltest + " " + sqltest1 + "";
