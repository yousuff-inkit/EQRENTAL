package com.dashboard.leaseagreement.lacreateeasy;
...
public class ClsLACreateEasyDAO {
	...
	public JSONArray getLAData(String uptodate,String cldocno,String branch,String id) throws SQLException{
		...
		try{
			...
			strsql="select rs.fleetnos fleet_no,d.sr_no masterrefsrno,ac.cldocno,m.doc_no,m.voc_no,ac.refname,m.po,m.refno,brd.brand_name brand,model.vtype model,coalesce(spec.name,'') spec,d.duration,"+
			...
			" (d.modelid=model.doc_no) left join gl_vehspec spec on d.specid=spec.doc_no left join gl_rsrveh rs on (d.doc_no=rs.rdocno) where m.status=3  and (d.qty-d.outqty<>0)"+sqltest;
