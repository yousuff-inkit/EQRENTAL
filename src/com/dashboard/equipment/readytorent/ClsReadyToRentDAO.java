...
public class ClsReadyToRentDAO {
	...
	public JSONArray getBranchwiseData(String check) throws SQLException
	{
		...
		try{
			...
			" if(veh.a_br=9,count(*),0) branch8,if(veh.a_br=10,count(*),0) branch9 from gl_equipmaster veh left join gl_status st on (veh.tran_code=st.status)"+
			...
			" else veh.a_br=10 end branch9 from gl_equipmaster veh inner join (select max(doc_no) doc,fleet_no from gl_emove group by fleet_no) v"+
			" on v.fleet_no=veh.fleet_no inner join gl_emove vm on vm.doc_no=v.doc left join gl_rentalcontractm rag on rag.doc_no=vm.rdocno and"+
			" vm.rdtype='ERC' and veh.tran_code='RA' left join gl_lagmt lag on lag.doc_no=vm.rdocno and vm.rdtype='LAG' and veh.tran_code='LA'"+
	...
	public ClsReadyToRentBean getPrint(String brnchval) throws SQLException {
		...
		try{
			...
			String sql="select * from (select ST.ST_DESC,convert(count(*),char(10)) VALUE,VEH.tran_code from gl_equipmaster VEH" + 
                   ...
                   "  union all"  +
                    "  select  ST.ST_DESC,count(*) value,veh.tran_code from gl_equipmaster veh" + 
