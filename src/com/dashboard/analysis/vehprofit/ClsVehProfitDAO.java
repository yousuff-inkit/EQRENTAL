		...
		try {
			...
			if(!hidfleet.equalsIgnoreCase("")){
				...
				if(gridtype.equalsIgnoreCase("detail")){
					...
					strSql="select fleetno,concat(veh.reg_no,' - ',plt.code_name) reg_no,veh.flname,brd.brand_name brand,model.vtype model,veh.prch_cost purchase_cost,grp.gname,yom.yom,CONVERT(concat(agmttype,' - ',agmtvocno) , CHAR(100)) agmtdetails,sum(raincome) raincome,sum(inv-raincome) ltincome,sum(inv) netincome,sum(lab) labcost,sum(sp) partscost,sum(dep) depcost,sum(exp-(lab+sp+dep)) otherexp,sum(exp) netexp,sum(inv-exp) netamount"
						+" from ("
						+" select case when jv.rtype='RAG' then ragmt.voc_no when jv.rtype='LAG' then lagmt.voc_no else '' end agmtvocno,jv.rtype agmttype,0.0 as raincome, if(cost.acno=(select hd.doc_no from my_account ac inner join my_head hd on ac.acno=hd.doc_no where ac.codeno='MAINTLAB'),if(cost.amount<0,cost.amount*-1,cost.amount),0.0) lab,"
						...
						+" left join gl_lagmt lagmt on ( jv.rdocno=lagmt.doc_no and jv.rtype='LAG')"
						+" left join gl_ragmt ragmt on ( jv.rdocno=ragmt.doc_no and jv.rtype='RAG')"
						+" where 1=1 and cost.costtype=6  "+sqltest+""
						+" union all"
						+" select case when jv.rtype='RAG' then ragmt.voc_no when jv.rtype='LAG' then lagmt.voc_no else '' end agmtvocno,jv.rtype agmttype,if(cost.acno=(select acno from gl_invmode where idno=1),if(cost.amount<0,cost.amount*-1,cost.amount),0.0) raincome,0.0 as lab,0.0 as sp,0.0 dep,cost.acno,head.description,if(cost.amount<0,cost.amount*-1,cost.amount) inv ,0.0 as exp,cost.jobid as fleetno"
						+" from my_costtran cost left join my_jvtran jv on(cost.tranid=jv.tranid)"
						...
						+" left join gl_vehplate plt on veh.pltid=plt.doc_no where 1=1 "+sqlcommon+" group by fleetno;";
				}
				else{
