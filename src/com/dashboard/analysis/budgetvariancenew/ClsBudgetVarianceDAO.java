package com.dashboard.analysis.budgetvariancenew;
...
public class ClsBudgetVarianceDAO  { 
	...
	public JSONArray budgetVarianceSumGridLoading(String branch,String fromdate,String todate,String type,String accdocno) throws SQLException {
		...
		try {
				...
				String sql = " select act.months, act.year, sum(inc) inc,  sum(actincome) actincome, sum(actincome - inc) varincome, sum(exp) exp, sum(actexpense) actexpense,"
						+ " sum(actexpense-exp) varexp  , sum(inc-exp) bporfit,sum(actincome-actexpense) profit,sum((actincome-actexpense)-(inc-exp)) vprofit"
				+ " from (select month,yeaR,DATE_FORMAT(concat(year,'-',month,'-01'),'%b %Y') months,SUM(IF(GRTYPE=5,AMOUNT,0)) inc,"
				+ " SUM(IF(GRTYPE=4,AMOUNT,0)) exp,0 actincome,0 actexpense from my_budgetdetail where month between month('"+sqlFromDate+"') and"
				+ " month('"+sqlToDate+"') and year between year('"+sqlFromDate+"') and year('"+sqlToDate+"') group  by yeaR,MONTH  union all"
				+ " select month(j.date) month,year(j.date) year,DATE_FORMAT(LAST_DAY(date(j.date)),'%b %Y') months,0 inc,0 exp ,"
				+ " sum(if(gr_type=5,dramount*-1,0)) actincome, sum(if(gr_type=4,dramount,0)) actexpense  from my_jvtran j left join my_head m"
				+ " on m.doc_no=j.acno where j.status=3 and j.date between '"+sqlFromDate+"' and  last_day('"+sqlToDate+"') group by year(j.date),month(j.date)) act  group by year,month";
			...
			/*	String sql = "select a.account,a.description accountname,a.acno,CONVERT(if(sum(a.budget)=0,' ',round(sum(a.budget),2)),CHAR(100)) budgetamount,CONVERT(if(sum(a.actualamount)=0,' ',round(sum(a.actualamount),2)),CHAR(100)) actualamount,"
				...
				*/
				String sql="select gr_type,a.account,a.description accountname,a.acno,CONVERT(if(sum(a.budget)=0,' ',round(sum(a.budget),2)),CHAR(100))"
						...
						+ " left join my_head h on j.acno=h.doc_no where j.status=3 and j.date between '"+sqlFromDate+"' and  last_day('"+sqlToDate+"')  and h.gr_type in (4,5) UNION ALL"
						+ " select h.account,h.description,d.acno,if(grtype=4,d.amount*-1,d.amount) budget,0 actualamount,h.gr_type from my_budgetm m"
						+ " left join my_budgetdetail d on m.doc_no=d.rdocno left join my_head h on d.acno=h.doc_no where m.status=3"
						+ " and d.month between month('"+sqlFromDate+"') and month('"+sqlToDate+"')  and d.year between year('"+sqlFromDate+"') and year('"+sqlToDate+"')  "
						+ " and h.gr_type in (4,5)) a group by a.acno having sum(a.actualamount)!=0 or sum(a.budget)!=0 "
						...
						+ " left join my_head h on j.acno=h.doc_no where j.status=3 and j.date between '"+sqlFromDate+"' and  last_day('"+sqlToDate+"') and"
						+ " h.gr_type in (4,5) UNION ALL select h.account,h.description,d.acno,if(grtype=4,d.amount*-1,d.amount) budget,0 actualamount,h.gr_type from my_budgetm m"
						+ " left join my_budgetdetail d on m.doc_no=d.rdocno left join my_head h on d.acno=h.doc_no where m.status=3 and"
						+ " d.month between month('"+sqlFromDate+"') and month('"+sqlToDate+"')  and d.year between year('"+sqlFromDate+"') and year('"+sqlToDate+"') and h.gr_type in (4,5)) b group by b.gr_type order by gr_type desc,account";
...
//				System.out.println("detailsql--------->>>"+sql);     
	...
	public JSONArray budgetVariancSumExcelExport(String branch,String fromdate,String todate,String type,String accdocno) throws SQLException {
		...
		String sql = " select act.months 'Month', sum(inc) 'Budget Income',sum(actincome) 'Actual Income', sum(actincome-inc) 'Variance Income', sum(exp) 'Expenditure',sum(actexpense) 'Actual Expenditure',sum(actexpense-exp) 'Variance Expenditure',"
				+ " sum(inc-exp) 'Budget Profit',sum(actincome-actexpense) 'Profit',sum((actincome-actexpense)-(inc-exp)) 'Variance Profit' "
		+ " from (select month,yeaR,DATE_FORMAT(concat(year,'-',month,'-01'),'%b %Y') months,SUM(IF(GRTYPE=5,AMOUNT,0)) inc,"
		+ " SUM(IF(GRTYPE=4,AMOUNT,0)) exp,0 actincome,0 actexpense from my_budgetdetail where month between month('"+sqlFromDate+"') and"
		+ " month('"+sqlToDate+"') and year between year('"+sqlFromDate+"') and year('"+sqlToDate+"') group  by yeaR,MONTH  union all"
		+ " select month(j.date) month,year(j.date) year,DATE_FORMAT(LAST_DAY(date(j.date)),'%b %Y') months,0 inc,0 exp ,"
...
*/  
			String sql="select kk.account 'Account',kk.description 'Account Name',kk.budgetamount 'Forecast',kk.actualamount 'Actual',kk.varianceamount 'Variance' from "
					...
					+ " h.gr_type in (4,5) UNION ALL select h.account,h.description,d.acno,if(grtype=4,d.amount*-1,d.amount) budget,0 actualamount,h.gr_type from my_budgetm m"
					+ " left join my_budgetdetail d on m.doc_no=d.rdocno left join my_head h on d.acno=h.doc_no where m.status=3 and"
					+ " d.month between month('"+sqlFromDate+"') and month('"+sqlToDate+"')  and d.year between year('"+sqlFromDate+"') and year('"+sqlToDate+"') and h.gr_type in (4,5)) b group by b.gr_type order by gr_type desc,account)kk";
