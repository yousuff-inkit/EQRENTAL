...
public class ClsVehicleAssetRegister  { 
	    ...
	    try {
				...
				if(check.equalsIgnoreCase("1")){
				 ...
				 sql = "SELECT m.fleet_no,m.flname,m.reg_no,m.plate,m.yom,m.age,m.purdate,m.deprper,\r\n" + 
			    		...
			    		"0.00 depr_add,0.00 depr_del,v.flname,v.reg_no,v.prch_dte,v.fstatus,v.vgrpid,v.pltid,v.vmodid,v.depr deprper,v.yom,a.brhid from gc_assettran a\r\n" + 
			    		"left join gl_vehmaster v on a.fleet_no=v.fleet_no where acno=(select acno from\r\n" + 
			    		"my_account where codeno='VEH')   and a.transtype='T' "+sql1+"\r\n" + 
			    		"UNION ALL\r\n" + 
			    		...
			    		"v.flname,v.reg_no,v.prch_dte,v.fstatus,v.vgrpid,v.pltid,vmodid,v.depr deprper,v.yom,a.brhid from gc_assettran a\r\n" + 
			    		"left join gl_vehmaster v on a.fleet_no=v.fleet_no where acno=(select acno from\r\n" + 
			    		"my_account where codeno='VAD')   and a.transtype='T' "+sql1+"\r\n" + 
			    		") d  left join gl_vehplate p on d.pltid=p.doc_no left join gl_yom yom on d.yom=yom.doc_no \r\n" + 
			    		"left join gl_vsaled sd on sd.fleetno=d.fleet_no left join gl_vsalem sm on sm.doc_no=sd.rdocno and sm.status=3  "
			    		+ "left join gl_vsaleretm srm on sM.doc_no=srm.salesinvdocno and srm.status=3 "
			    		+ " left join gl_vsaleretd srd on SRM.DOC_NO=SRD.RDOCNO AND srd.fleetno=Sd.fleetno "
			    		+ " where coalesce(sm.date,\r\n" + 
			    		"'"+sqlFromDate+"')>='"+sqlFromDate+"' AND SRD.FLEETNO IS NULL  and d.prch_dte<='"+sqlToDate+"'"+sql+" group by d.fleet_no"+strSql+") l ) m";
	...
	public JSONArray vehicleDetailedAsssetGridLoading(String branch,String fromdate,String todate,String fleetNo) throws SQLException {
	    ...
	    try {
				...
				//System.out.println("=====grid load qry2==="+sql);
				ResultSet resultSet = stmtBVAR.executeQuery(sql);
	    ...
	    try {
		    ...
		    /*sql="select l.*,CONVERT(if((l.asset_total-l.depr_total)=0,'',(round((l.asset_total-l.depr_total),2))),CHAR(100)) 'NET TOTAL' from ("
			 ...
			 sql = "SELECT m.fleet_no 'FLEET NO',m.flname 'FLEET NAME',m.reg_no 'REG NO',m.plate PLATE,m.yom 'YoM',TRUNCATE(m.age,0) 'AGE(M)',m.deprper 'Depr(%)',m.purdate 'PUR. DATE',\r\n" + 
			    		...
			    		"if(a.date>='"+sqlFromDate+"' and a.date<='"+sqlToDate+"' AND a.dramount>0,a.dramount,'') asset_add,\r\n" + 
			    		"if(a.date>='"+sqlFromDate+"' and a.date<='"+sqlToDate+"' AND a.dramount<0,a.dramount,'') asset_del,0.00 depr_opn,\r\n" + 
			    		"0.00 depr_add,0.00 depr_del,v.flname,v.reg_no,v.prch_dte,v.fstatus,v.vgrpid,v.vmodid,v.depr deprper,a.brhid,p.code_name,yom.yom from gc_assettran a\r\n" + 
			    		"left join gl_vehmaster v on a.fleet_no=v.fleet_no left join gl_vehplate p on v.pltid=p.doc_no left join gl_yom yom on v.yom=yom.doc_no where acno=(select acno from\r\n" + 
			    		"my_account where codeno='VEH')   and a.transtype='T' "+sql1+" \r\n" + 
			    		"UNION ALL\r\n" + 
			    		"select a.fleet_no,a.date,0.00 asset_opn,0.00 asset_add,0.00 asset_del,if(a.date<'"+sqlFromDate+"',a.dramount,0.00) depr_opn,\r\n" + 
			    		"if(a.date>='"+sqlFromDate+"' and a.date<='"+sqlToDate+"' AND a.dramount<0,a.dramount,'') depr_add,\r\n" + 
			    		"if(a.date>='"+sqlFromDate+"' and a.date<='"+sqlToDate+"' AND a.dramount>0,a.dramount,'')  depr_del,\r\n" + 
			    		"v.flname,v.reg_no,v.prch_dte,v.fstatus,v.vgrpid,v.vmodid,v.depr deprper,a.brhid,p.code_name,yom.yom from gc_assettran a\r\n" + 
			    		"left join gl_vehmaster v on a.fleet_no=v.fleet_no left join gl_vehplate p on v.pltid=p.doc_no  left join gl_yom yom on v.yom=yom.doc_no where acno=(select acno from\r\n" + 
			    		"my_account where codeno='VAD')   and a.transtype='T' "+sql1+"\r\n" + 
			    		") d\r\n" + 
			    		"left join gl_vsaled sd on sd.fleetno=d.fleet_no inner join gl_vsalem sm on sm.doc_no=sd.rdocno and sm.status=3  where  coalesce(sm.date,\r\n" + 
