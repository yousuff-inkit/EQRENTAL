package com.limousine.jobassignmentmultiple;
...
public class ClsJobAssignMultipleDAO {
	...
	public JSONArray getDetailData(String id,String branch) throws SQLException{
		...
		try{
			...
			" brd on tran.brandid=brd.doc_no left join gl_vehmodel model on (tran.modelid=model.doc_no) left join gl_limomanagement mgmt on (tran.bookdocno=mgmt.docno and tran.docname=mgmt.job) where book.status=3 and book.detailstatus<>3 and"+
			" (tran.assignstatus=0) and (tran.transferbranch=0 or tran.transferbranch="+branch+") and tran.masterstatus<>4 and mgmt.bstatus<>7 union all"+
			" select book.voc_no bookvocno,coalesce(book.locationtype,'') loctype,coalesce(case when book.locationtype='flight' then book.flightno else book.roomno end,'') locrefno,coalesce(case when book.locationtype='flight' then book.airport else book.hotel end,'') locrefname,hours.transfertype,book.doc_no,book.cldocno,book.guestno,hours.doc_no detaildocno,hours.brandid,hours.modelid,hours.docname,ac.refname,guest.guest,'Limo' type,hours.blockhrs,hours.pickupdate,"+
			...
			" brd on (hours.brandid=brd.doc_no) left join gl_vehmodel model on (hours.modelid=model.doc_no) left join gl_limomanagement mgmt on (hours.bookdocno=mgmt.docno and hours.docname=mgmt.job) where book.status=3 and (hours.assignstatus=0) and (hours.transferbranch=0 or hours.transferbranch="+branch+") and hours.masterstatus<>4 and mgmt.bstatus<>7 ";
	...
	public JSONArray getFleetData(String fleetno,String fleetname,String brand,String model,String group,String allfleets,
			String gridbrandid,String gridmodelid,String id,String regno) throws SQLException{
	...
	public ArrayList<String> insert(Date sqluptodate, String cmbprocess, String hiddriver,
		...
		try{
			...
			for(int i=0;i<jobarray.size();i++){
				...
				if(docno<=0){
					...
					if(cmbprocess.equalsIgnoreCase("1")){
						...
						int assignval=stmtassign.executeUpdate(strassignupdate);
						System.out.println("AssignValue:"+assignval);
						if(assignval<=0){
							errorstatus=1;
							return docarray;
						}
					
						String strupdatelimomgmt="update gl_limomanagement set bstatus=2,assigntype=3,drivername='"+name+"' where docno="+bookdocno+" and job='"+jobname+"'";
			...
			if(errorstatus==0){
				...
				System.out.println("User Defined Error Occured");
			}
			}
