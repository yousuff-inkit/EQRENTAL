...
docval=0;  
		...
		for(int i=0;i< descarray.size();i++){
		  ...
		  int taxaccount=0,taxaccount2=0;
		 ...
		 if(typedocval>0)
		 {
			 ...
			 if(cmbbilltype==2)
			 {
			 String sqlss2="select p.doc_no,p.producttype ptype,m.per,m.acno taxaccount from my_ptype p "
						+" left join gl_taxmaster m on p.doc_no=m.typeid and m.type=2 where  m.type=2 and  p.doc_no="+ptype+" group by p.doc_no";
					 
					ResultSet rs12=stmt11.executeQuery(sqlss2) ;
			 if(rs12.first())
			 {
				 taxaccount2=rs12.getInt("taxaccount");
		 ...
		 //System.out.println("taxtotal==="+typedramt);
		 ...
		 double dramt=(nettot)*1;
			...
			if(cmbbilltype==2)
			{
				dramt=dramt-typedramt;
				ldramt=ldramt-typedramt;
			}
		...
		//	System.out.println(sql1);
			...
			//	System.out.println(sql1);
				 ...
				 if(cmbbilltype==2)
				 {
				 					 String sql12="insert into my_jvtran(date,ref_detail,doc_no,acno,description,curId,rate,dramount,ldramount,out_amount,id,trtype,ref_row,LAGE,cldocno,lbrrate,costtype,costcode,dTYPE,brhId,tr_no,STATUS)   "
				 						 		+ "values('"+sqlStartDate+"','"+refdetails+"',"+docno+",'"+taxaccount2+"','"+jvdesc+"','"+cmbcurr+"','"+currate+"',round("+typedramt+",2),round("+typeldramt+",2),0,1,0,0,0,0,0,0,0,'CPR','"+session.getAttribute("BRANCHID").toString()+"',"+tranno+",'"+iapprovalStatus+"')";
				 						 
				 					//	System.out.println(sql1);
				 						 int sss1 = stmtnipurchase.executeUpdate(sql12);
				 }
				...
				String selectsqls= "select rate,sum(a.nettaxamount) nettaxamount,sum(a.total1) total1,sum(a.total2) total2,sum(a.total3) total3, "
						...
						+"  from ( "
					+" 	 select m.rate, d.nettotal+coalesce(d.taxamount,0) nettaxamount,if(coalesce(d.taxamount,0)>0,d.nettotal,0) total1, "
						...
						+"  0 tax7,  0 tax8, "
						+"  0 tax9,  0 tax10 "
						 +"  from my_srvpurretd d left join my_srvpurretm m on m.doc_no=d.rdocno where rdocno="+docno+" ) a" ;
	...
	/*     int resultSet4 = stmtnipurchase.executeUpdate(sql);
		...
		//	 System.out.println("taxtotal==="+taxtotal);
			 double dramt=(nettot)*1;
				...
				if(cmbbilltype==2)
				{
					dramt=dramt-taxtotal;
					ldramt=ldramt-taxtotal;
				}
				 String sql1="insert into my_jvtran(date,ref_detail,doc_no,acno,description,curId,rate,dramount,ldramount,out_amount,id,trtype,ref_row,LAGE,cldocno,lbrrate,costtype,costcode,dTYPE,brhId,tr_no,STATUS)   "
				 		+ "values('"+sqlStartDate+"','"+refdetails+"',"+docno+",'"+accdoc+"','"+jvdesc+"','"+cmbcurr+"','"+currate+"',round("+dramt+",2),round("+ldramt+",2),0,1,0,0,0,0,0,0,0,'CPR','"+session.getAttribute("BRANCHID").toString()+"',"+tranno+",'"+iapprovalStatus+"')";
	  ...
	  java.sql.Date  sqlStartDate = null;
		...
		try {
	      ...
	      String pySql=("select coalesce(m.typeid,0) typeid,coalesce(p.ptype,'') ptype,coalesce(p.per,0) per  ,m.doc_no,m.voc_no,m.date,"
	      		+ " (m.netamount-m.roundof) netamount,m.invno,m.invdate,m.type,m.acno,m.refno,m.curid,m.rate,m.delterm,m.payterm,m.deldate,m.desc1,h.description,h.account,m.billtype billtypeid,if(m.billtype='1','VAT','RCM') as billtype "
