...
public class ClsVendorDetailsAction extends ActionSupport {
	...
	private int chkrcm, hidchkrcm;

	public int getChkrcm() {
		return chkrcm;
	}

	public void setChkrcm(int chkrcm) {
		this.chkrcm = chkrcm;
	}

	public int getHidchkrcm() {
		return hidchkrcm;
	}

	public void setHidchkrcm(int hidchkrcm) {
		this.hidchkrcm = hidchkrcm;
	}
	...
	public String saveAction() throws ParseException, SQLException {
		...
		if (mode.equalsIgnoreCase("A")) {
			int val = vendorDetailsDAO.insert(vendorDate, getFormdetailcode(), getTxtvendorname(), getCmbcurrency(),
					getCmbcategory(), getCmbtype(), getTxtregisteredtrnno(), getCmbaccgroup(), getTxtaccount(),
					getTxtcredit_period_min(), getTxtcredit_period_max(), getTxtcredit_limit(), getTxtaddress(),
					getTxtaddress1(), getTxttel(), getTxtmob(), getTxtoffice(), getTxtfax(), getTxtemail(),
					getTxtcontact(), getTxtextno(), getChkrcm(), session, request);
			if (val > 0.0) {
			...
			int Status = vendorDetailsDAO.edit(getTxtvendordocno(), getFormdetailcode(), vendorDate, getTxtvendorname(),
					getCmbcurrency(), getCmbcategory(), getCmbtype(), getTxtregisteredtrnno(), getCmbaccgroup(),
					getTxtaccount(), getTxtcredit_period_min(), getTxtcredit_period_max(), getTxtcredit_limit(),
					getTxtaddress(), getTxtaddress1(), getTxttel(), getTxtmob(), getTxtoffice(), getTxtfax(),
					getTxtemail(), getTxtcontact(), getTxtextno(), getChkrcm(), session);
			if (Status > 0) {
			...
			setChkrcm(vendorDetailsbean.getChkrcm());
			setHidchkrcm(vendorDetailsbean.getChkrcm());
