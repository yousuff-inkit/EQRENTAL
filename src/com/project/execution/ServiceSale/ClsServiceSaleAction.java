...
import com.opensymphony.xwork2.ActionSupport;
	...
	public class ClsServiceSaleAction extends ActionSupport{
		...
		private String nipurchasedate,username,email,salesman;    
		...
		private String refno,acctype,nipuraccid,puraccname,cmbcurr,hidcmbcurr,currate,deliverydate,hiddeliverydate,delterms,payterms,purdesc,mode,msg, cmbcurrval,acctypeval,accdocno,formdetailcode,brchName;;
		...
		private String lblctcperson,lblclientemail,lblsitedetails;
	...
	public void setLblbankibanno(String lblbankibanno) {
		...
		public String getLblsitedetails() {
			return lblsitedetails;
		}
		
		public void setLblsitedetails(String lblsitedetails) {
			this.lblsitedetails = lblsitedetails;
		}
	...
	if(mode.equalsIgnoreCase("A")){
		...
		int val=purchaseDAO.insert(sqlStartDate1,sqlpurdeldate,getNireftype(),getRefno(),getAcctype(),getAccdocno(),getPuraccname(), 
				getCmbcurr(),getCurrate(),getDelterms(),getPayterms(),getPurdesc(),session,getBrchName(),getMode(),getNettotal(),descarray,getFormdetailcode(),
		...
		if(val>0){
			...
			boolean Status=purchaseDAO.edit(getMasterdoc_no(),sqlStartDate1,sqlpurdeldate,getNireftype(),getRefno(),
					...
					getPurdesc(),session,getBrchName(),getMode(),getNettotal(),descarray,getFormdetailcode(),getTarannumber(),request,sqlinvdate,getInvno(),getInvDate(),getInterstate(),getTaxperc());
			if(Status){
			...
			boolean Status=purchaseDAO.delete(getMasterdoc_no(),session,getBrchName(),getMode(),getFormdetailcode());
		if(Status){
			setOrdermasterdoc_no(getOrdermasterdoc_no()) ;
			setRefno(getRefno());
			setReftypeval(getNireftype());
			 ...
			 int doc=Integer.parseInt(request.getParameter("docno"));
			  ...
			  //cl details
					setLblvenaddress(pintbean.getLblvenaddress());
					 ...
					 //System.out.println("trn=="+pintbean.getLblcltrnno());
                    ...
                    setSalesman(pintbean.getSalesman());  
...
//			      System.out.println("termsqueryyyyyyyyyyy"+termsquery);
			           ...
			           String descQry=" select @i:=@i+1 as srno,a.* from  (select coalesce(q.reg_no,'') item,coalesce(c.code,'') code,d.srno,coalesce(d.remarks,'') hireperiod,coalesce(q.flname,d.desc1) description,round(d.unitprice,2) unitprice,  round(d.qty) qty,round(d.total,2) amount,round(d.discount,2) discount,round(d.nettotal,2) nettotal,d.nuprice, round(d.taxper,2) tax,round(d.tax,2) taxper,TRUNCATE(d.tax,2) taxamount,round(d.nettaxamount,2) netamt "
				           		+ " from my_srvsaled d left join my_head h on h.doc_no=d.acno left join my_srvsalem m on m.doc_no=d.rdocno left join my_curr c on m.curid=c.doc_no left join gl_equipmaster q on d.costcode=q.fleet_no  where d.rdocno="+docno+"  ) a,(select @i:=0) r;";			          
...
//			          System.out.println("adrsss+++++++++"+bean.getLblvenaddress());
			          ...
			          String strSql1 = "select name,address,beneficiary,account,ibanno,swiftcode from cm_bankdetails where status=3 and doc_no="+bankdocno+"";
						...
						String banknames="",bankaddress="",bankbeneficiary="",bankaccount="",bankibanno="",bankswiftcode="";
						while(rs1.next()) {
				        ...
				        String strSql2 = "select coalesce(cm.description,'')description, coalesce(calc.address,'')address " + 
				          		"from gl_rentalcontractm cm left join gl_rentalquotecalc calc on calc.contractdocno=cm.doc_no " + 
				          		"where cm.doc_no="+bean.getRefno();
							
			          param.put("clientpayterm", bean.getLblcreditperiod());
...
//			          System.out.println("Email====="+bean.getEmail());
			          ...
			          param.put("sitedetails", bean.getLblsitedetails());
			        ...
			        //  System.out.println("netamt"+ bean.getLblnettaxamount());
			         ...
			         // System.out.println("trnfirst======"+bean.getLblcltrnno());
			         // System.out.println("trnno===="+pintbean.getLblcltrnno());
...
//			          System.out.println("headerstatus"+header);
			          ...
			          //System.out.println("trn down===="+bean.getLblcltrnno());
			          ...
			          param.put("salesman", bean.getSalesman());   
