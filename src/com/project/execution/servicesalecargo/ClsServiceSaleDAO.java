...
import com.project.execution.servicesalecargo.ClsServiceSaleDAO;
		    ...
		    if(!(purorderarray[1].trim().equalsIgnoreCase("undefined")|| purorderarray[1].trim().equalsIgnoreCase("NaN")||purorderarray[1].trim().equalsIgnoreCase("")|| purorderarray[1].isEmpty()))
		     {
	    		 ...
	    		 String sql="INSERT INTO my_srvsaled(srno,qty,desc1,unitprice,total,discount,nettotal,taxper,tax,nettaxamount,nuprice,costtype,costcode,remarks,acno,curid,rate,brhid,rdocno)VALUES"
					       ...
					       + "'"+(purorderarray[16].trim().equalsIgnoreCase("undefined") || purorderarray[16].trim().equalsIgnoreCase("NaN")||purorderarray[16].trim().equalsIgnoreCase("")|| purorderarray[16].isEmpty()?1:purorderarray[16].trim())+"',"
					       + "'"+(purorderarray[17].trim().equalsIgnoreCase("undefined") || purorderarray[17].trim().equalsIgnoreCase("NaN")||purorderarray[17].trim().equalsIgnoreCase("")|| purorderarray[17].isEmpty()?1:purorderarray[17].trim())+"',"
...
public boolean edit(int docno,Date sqlStartDate,Date purdeldate, String reftype, String refno, String acctype,
	...
	try{
		System.out.println("in cargo edit");
		 conn=connDAO.getMyConnection();
		 conn.setAutoCommit(false);
...
public   JSONArray reloadnipurchase(HttpSession session,String nidoc) throws SQLException {
	  ...
	  return RESULTDATA;
		...
		try {
	        	...
	        	String pySql=(" select d.curid,d.rate,d.srno,d.desc1 description,d.unitprice,d.qty,d.qty qutval,d.total,d.discount,d.nettotal,d.nuprice,d.acno headdoc,h.gr_type grtype ,"
   ...
   e.printStackTrace();
	...
	public  ClsServiceSaleBean getPrint(int docno, HttpServletRequest request ,HttpSession session) throws SQLException {
		...
		try {
				...
				*/
				String resql = ("select  ar.area,cr.country_name as country,year(curdate())curyear,m.voc_no,if(CONCAT(if(a.per_tel is null or trim(a.per_tel)='',if(a.per_mob is null or trim(a.per_mob)='',if(a.com_mob is null or trim(a.com_mob)='',' NIL , ',CONCAT(a.com_mob,' , ')),CONCAT(a.per_mob,' , ')),CONCAT(a.per_tel,' , ')), "
