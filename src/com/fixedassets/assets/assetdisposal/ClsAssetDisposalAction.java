...
import com.fixedassets.assets.assetssalesinvoice.ClsAssetSalesInvBean;
import com.fixedassets.assets.assetssalesinvoice.ClsAssetSalesInvDAO;

public class ClsAssetDisposalAction {
	ClsCommon ClsCommon=new ClsCommon();

	ClsAssetDisposalDAO assetdao=new ClsAssetDisposalDAO();
	ClsAssetDisposalBean bean;
	private int docno;
	private int vocno;
	private String date;
...
public void setLblamountwords(String lblamountwords) {
	...
	public String saveAction() throws ParseException, SQLException{
		...
		if(mode.equalsIgnoreCase("A")){
			...
			ClsAssetDisposalBean bean=assetdao.insert(sqlStartDate,getCmbtype(),getDescription(),session,getMode(),assetarray,
								getFormdetailcode(),"0","0",getDays(),getBrchName(),request);
						if(bean.getDocno()>0){
...
public String printAction() throws SQLException{
		...
		setLblprintname("Asset Disposal");
