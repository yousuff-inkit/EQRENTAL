...
/*				    			 
				    		   ...
				    		   */
						             ...
						             String insql="INSERT INTO my_srvd(sr_no,psrno,prdId,unitid,qty,amount,total,discount,nettotal,specno,disper,stockid,tr_no,rdocno,taxper,taxamount,nettaxamount,taxdocno,fr)VALUES"
								       ...
								       + "'"+(detmasterarrays[4].trim().equalsIgnoreCase("undefined") || detmasterarrays[4].trim().equalsIgnoreCase("NaN")||detmasterarrays[4].trim().equalsIgnoreCase("")|| detmasterarrays[4].isEmpty()?0.0:ClsCommon.Round(Double.parseDouble(detmasterarrays[4].trim()),2))+"',"
								       + "'"+(detmasterarrays[5].trim().equalsIgnoreCase("undefined") || detmasterarrays[5].trim().equalsIgnoreCase("NaN")||detmasterarrays[5].trim().equalsIgnoreCase("")|| detmasterarrays[5].isEmpty()?0.0:ClsCommon.Round(Double.parseDouble(detmasterarrays[5].trim()),2))+"',"
								       ...
								       + "'"+(detmasterarrays[7].trim().equalsIgnoreCase("undefined") || detmasterarrays[7].trim().equalsIgnoreCase("NaN")||detmasterarrays[7].trim().equalsIgnoreCase("")|| detmasterarrays[7].isEmpty()?0.0:ClsCommon.Round(Double.parseDouble(detmasterarrays[7].trim()),2))+"',"
								       ...
								       + "'"+(detmasterarrays[18].trim().equalsIgnoreCase("undefined") || detmasterarrays[18].trim().equalsIgnoreCase("NaN")||detmasterarrays[18].trim().equalsIgnoreCase("") || detmasterarrays[18].isEmpty()?0.0:ClsCommon.Round(Double.parseDouble(detmasterarrays[18].trim()),2))+"',"
								       + "'"+(detmasterarrays[19].trim().equalsIgnoreCase("undefined") || detmasterarrays[19].trim().equalsIgnoreCase("NaN")||detmasterarrays[19].trim().equalsIgnoreCase("")  || detmasterarrays[19].isEmpty()?0.0:ClsCommon.Round(Double.parseDouble(detmasterarrays[19].trim()),2))+"',"
				    		 ...
				    		 else if(reftype.equalsIgnoreCase("PO"))
				    		 {
							 			    ...
							 			    String	foc=""+(detmasterarrays[14].trim().equalsIgnoreCase("undefined") || detmasterarrays[14].trim().equalsIgnoreCase("NaN")||detmasterarrays[14].trim().equalsIgnoreCase("")|| detmasterarrays[14].isEmpty()?0:detmasterarrays[14].trim())+"";	 
												     ...
												     String sql2="insert into my_prddin(sr_no, psrno,prdid,specno,unit_price,op_qty,foc,locid,brhid,tr_no,dtype,cost_price,invno,pstatus,date,fr,unitid)values"
														     ...
														     +"'"+opqty*fr+"','"+focqty*fr+"',"
												             +"'"+locationid+"','"+session.getAttribute("BRANCHID").toString()+"','"+mastertr_no+"','"+formdetailcode+"',"+(costprice/fr)*venrate+",'"+docno+"',1,'"+masterdate+"',"+fr+","+unitids+")";
						...
						//	System.out.println("---out_qty-----"+out_qty);	
							...
							//	System.out.println("--1---sqls---"+sqls);
                              ...
                              //	String prdinsqls="update my_prddin set cost_price="+costprice+",invno='"+docno+"',pstatus=1,unit_price='"+amounts+"' where tr_no="+tr_no+" and prdid="+prdid+" and  stockid="+stockid+"";
                              	...
                              	String prdinsqls="update my_prddin set cost_price="+costprice*venrate+",invno='"+docno+"',pstatus=1,unit_price='"+Double.parseDouble(amounts)+"' where tr_no="+tr_no+" and prdid="+prdid+" and  stockid="+stockid+"";
...
/*					
			 ...
			 String jvselect="SELECT sum(ldramount) dramount from my_jvtran where tr_no='"+mastertr_no+"'";
			  //  System.out.println("-----5--sqls3----"+sqls3) ;
			   ResultSet jvrs = stmt.executeQuery (jvselect);
						   ...
						   ResultSet exptass3 = stmt.executeQuery (expsqls3);
							...
							// System.out.println("---adjustsql----"+adjustsql) ;
								     ResultSet resultadj = stmt.executeQuery(adjustsql);
									  ...
									  jvdramount=ClsCommon.Round(jvdramount, 2);
									      adjustldramounts=ClsCommon.Round(adjustldramounts, 2);
				...
				String selectsqls="select rate, sum(coalesce(a.nettaxamount,0)+coalesce(a.nettotal,0)+coalesce(a.exnet,0))+roundval nettaxamount,if(sum(coalesce(a.totse,0)+coalesce(a.totma,0))=0,0,sum(coalesce(a.totse,0)+coalesce(a.totma,0))+roundval) total1,if(coalesce(sum(a.total2),0)=0,0,sum(a.total2)+roundval) total2,sum(a.total3) total3," 
						...
						+"from (select mm.rate,coalesce(mm.roundval,0) roundval,coalesce(ex.nettotal,0) exnet,sr.nettotal,sr.srvtaxamount,m.typeid,d.nettotal+coalesce(d.taxamount,0) nettaxamount,if(m.typeid=1 and d.taxamount>0,d.nettotal,0) totma,if(m.typeid=1 and sr.srvtaxper>0,(sr.nettotal-sr.srvtaxamount),0)totse, "
						...
						+" from my_srvd d left join my_srvm mm on mm.tr_no=d.tr_no left join my_main m on m.psrno=d.psrno left join my_srvser sr on d.tr_no=sr.tr_no left join my_srvexp ex on sr.tr_no=ex.tr_no where d.tr_no="+mastertr_no+" ) a " ;
				...
				if(rss101.first())
					...
					int result=ClsVatInsert.vatinsert(1,1,conn,mastertr_no,venderaccount,vocno,masterdate,formdetailcode,session.getAttribute("BRANCHID").toString(),invno,1,arr,mode)	;
		...
		//	System.out.println("sssssss"+docno);
			...
			double total = 0;
	        String sql="select sum(ldramount) as jvtotal from my_jvtran where tr_no="+mastertr_no;
	...
	public int update(int masterdoc_no, Date masterdate, Date deldate,
		...
		try{
		    ...
		    String strstockchk="select stockid from my_srvm m inner join  my_prddin i on m.tr_no=i.tr_no where (out_qty>0 or rsv_qty>0 or del_qty>0) and m.doc_no="+masterdoc_no;
           System.out.println("strstockchk==="+strstockchk);
		    ResultSet rsSTK=stmtSTK.executeQuery(strstockchk);
            if (rsSTK.next())
            {
			  ...
			  if(taxmethod>0)
			  {
				...
				updtax=stval+descgridtax;
				  if(updatadata.equalsIgnoreCase("Editvalue")) // change
					...
					netTotaldown=netTotaldown-stval; 
				    //   System.out.println("nettotaldown======="+netTotaldown+"stval==="+stval);
...
//					 } 
			...
			if(updatadata.equalsIgnoreCase("Editvalue"))
			{
									 			    ...
									 			    String	foc=""+(detmasterarrays[14].trim().equalsIgnoreCase("undefined") || detmasterarrays[14].trim().equalsIgnoreCase("NaN")||detmasterarrays[14].trim().equalsIgnoreCase("")|| detmasterarrays[14].isEmpty()?0:detmasterarrays[14].trim())+"";	 
														     ...
														     String sql2="insert into my_prddin(sr_no, psrno,prdid,specno,unit_price,op_qty,foc,locid,brhid,tr_no,dtype,cost_price,invno,pstatus,date,fr,unitid)values"
																     ...
																     + "'"+(detmasterarrays[10].trim().equalsIgnoreCase("undefined") || detmasterarrays[10].trim().equalsIgnoreCase("NaN")||detmasterarrays[10].trim().equalsIgnoreCase("")|| detmasterarrays[10].isEmpty()?0:detmasterarrays[10].trim())+"',"
														               + "'"+(detmasterarrays[4].trim().equalsIgnoreCase("undefined") || detmasterarrays[4].trim().equalsIgnoreCase("NaN")||detmasterarrays[4].trim().equalsIgnoreCase("")|| detmasterarrays[4].isEmpty()?0:detmasterarrays[4].trim())+"',"
																     +"'"+opqty*fr+"','"+focqty*fr+"',"
														             +"'"+locationid+"','"+session.getAttribute("BRANCHID").toString()+"','"+mastertr_no+"','"+formdetailcode+"',"+costprice*venrate+",'"+docno+"',1,'"+masterdate+"',"+fr+","+unitids+")";
				...
				/*		 String strSql="select dd.tr_no,dd.psrno,d.specno,sum(dd.op_qty) qty,sum((dd.op_qty-(dd.rsv_qty+dd.out_qty+dd.del_qty))) balstkqty,sum(dd.out_qty) out_qty,date,sum(d.out_qty) grnout from my_prddin dd "
						...
						//	System.out.println("---grnout-----"+grnout);	
                            ...
                            //  	String prdinsqls="update my_prddin set cost_price="+costprice+",invno='"+docno+"',pstatus=1,unit_price='"+amounts+"'  where tr_no="+tr_no+" and prdid="+prdid+" and  stockid="+stockid+"";
						      	String prdinsqls="update my_prddin set cost_price="+(costprice/fr)*venrate+",invno='"+docno+"',pstatus=1,unit_price='"+Double.parseDouble(amounts)/fr+"'  where tr_no="+tr_no+" and prdid="+prdid+" and  stockid="+stockid+"";		
			...
			for(int i=0;i< descarray.size();i++){
		     ...
		     String sql1="INSERT INTO my_srvser(srno,qty,desc1,unitprice,total,discount,nettotal,srvtaxper,srvtaxamount,brhid,rdocno,tr_no)VALUES"
				       ...
				       + "'"+(purorderarray[1].trim().equalsIgnoreCase("undefined") || purorderarray[1].trim().equalsIgnoreCase("NaN")|| purorderarray[1].trim().equalsIgnoreCase("")|| purorderarray[1].isEmpty()?0:purorderarray[1].trim())+"',"
				       + "'"+(purorderarray[2].trim().equalsIgnoreCase("undefined") || purorderarray[2].trim().equalsIgnoreCase("NaN")|| purorderarray[2].trim().equalsIgnoreCase("")|| purorderarray[2].isEmpty()?0:purorderarray[2].trim())+"',"
				       + "'"+(purorderarray[3].trim().equalsIgnoreCase("undefined") || purorderarray[3].trim().equalsIgnoreCase("NaN")||purorderarray[3].trim().equalsIgnoreCase("")|| purorderarray[3].isEmpty()?0.0:ClsCommon.Round(Double.parseDouble(purorderarray[3].trim()),2))+"',"
				       + "'"+(purorderarray[4].trim().equalsIgnoreCase("undefined") || purorderarray[4].trim().equalsIgnoreCase("NaN")||purorderarray[4].trim().equalsIgnoreCase("")|| purorderarray[4].isEmpty()?0.0:ClsCommon.Round(Double.parseDouble(purorderarray[4].trim()),2))+"',"
				       + "'"+(purorderarray[5].trim().equalsIgnoreCase("undefined") || purorderarray[5].trim().equalsIgnoreCase("NaN")||purorderarray[5].trim().equalsIgnoreCase("")|| purorderarray[5].isEmpty()?0.0:ClsCommon.Round(Double.parseDouble(purorderarray[5].trim()),2))+"',"
				       + "'"+(purorderarray[6].trim().equalsIgnoreCase("undefined") || purorderarray[6].trim().equalsIgnoreCase("NaN")||purorderarray[6].trim().equalsIgnoreCase("")|| purorderarray[6].isEmpty()?0.0:ClsCommon.Round(Double.parseDouble(purorderarray[6].trim()),2))+"',"
				       + "'"+(purorderarray[7].trim().equalsIgnoreCase("undefined") || purorderarray[7].trim().equalsIgnoreCase("NaN")||purorderarray[7].trim().equalsIgnoreCase("")|| purorderarray[7].isEmpty()?0.0:ClsCommon.Round(Double.parseDouble(purorderarray[7].trim()),2))+"',"
				       + "'"+(purorderarray[8].trim().equalsIgnoreCase("undefined") || purorderarray[8].trim().equalsIgnoreCase("NaN")||purorderarray[8].trim().equalsIgnoreCase("")|| purorderarray[8].isEmpty()?0.0:ClsCommon.Round(Double.parseDouble(purorderarray[8].trim()),2))+"',"
				       + "'"+session.getAttribute("BRANCHID").toString()+"',"
				       +"'"+docno+"','"+mastertr_no+"')";
			//  System.out.println("===sql1==="+sql1);
				     int resultSet2 = stmtpuchase.executeUpdate(sql1);
			...
			for(int i1=0;i1< exparray.size();i1++){
		    	
			     String[] singleexparray=exparray.get(i1).split("::");
			     if(!(singleexparray[1].trim().equalsIgnoreCase("undefined")|| singleexparray[1].trim().equalsIgnoreCase("NaN")||singleexparray[1].trim().equalsIgnoreCase("")|| singleexparray[1].isEmpty()))
			     {
		     ...
		     String sql="INSERT INTO my_srvexp(srno,qty,descdocno,unitprice,total,discount,nettotal,acno,brhid,tr_no,rdocno)VALUES"
				       ...
				       + "'"+(singleexparray[1].trim().equalsIgnoreCase("undefined") || singleexparray[1].trim().equalsIgnoreCase("NaN")|| singleexparray[1].trim().equalsIgnoreCase("")|| singleexparray[1].isEmpty()?0:singleexparray[1].trim())+"',"
				       + "'"+(singleexparray[2].trim().equalsIgnoreCase("undefined") || singleexparray[2].trim().equalsIgnoreCase("NaN")|| singleexparray[2].trim().equalsIgnoreCase("")|| singleexparray[2].isEmpty()?0:singleexparray[2].trim())+"',"
				       + "'"+(singleexparray[3].trim().equalsIgnoreCase("undefined") || singleexparray[3].trim().equalsIgnoreCase("NaN")||singleexparray[3].trim().equalsIgnoreCase("")|| singleexparray[3].isEmpty()?0.0:ClsCommon.Round(Double.parseDouble(singleexparray[3].trim()),2))+"',"
				       + "'"+(singleexparray[4].trim().equalsIgnoreCase("undefined") || singleexparray[4].trim().equalsIgnoreCase("NaN")||singleexparray[4].trim().equalsIgnoreCase("")|| singleexparray[4].isEmpty()?0.0:ClsCommon.Round(Double.parseDouble(singleexparray[4].trim()),2))+"',"
				       + "'"+(singleexparray[5].trim().equalsIgnoreCase("undefined") || singleexparray[5].trim().equalsIgnoreCase("NaN")||singleexparray[5].trim().equalsIgnoreCase("")|| singleexparray[5].isEmpty()?0.0:ClsCommon.Round(Double.parseDouble(singleexparray[5].trim()),2))+"',"
				       + "'"+(singleexparray[6].trim().equalsIgnoreCase("undefined") || singleexparray[6].trim().equalsIgnoreCase("NaN")||singleexparray[6].trim().equalsIgnoreCase("")|| singleexparray[6].isEmpty()?0.0:ClsCommon.Round(Double.parseDouble(singleexparray[6].trim()),2))+"',"
				       + "'"+(singleexparray[7].trim().equalsIgnoreCase("undefined") || singleexparray[7].trim().equalsIgnoreCase("NaN")||singleexparray[7].trim().equalsIgnoreCase("")|| singleexparray[7].isEmpty()?0:singleexparray[7].trim())+"',"
				       + "'"+session.getAttribute("BRANCHID").toString()+"',"
				       +"'"+mastertr_no+"','"+docno+"')";
		//  System.out.println("===sql==="+sql);
			...
			double stkvalue=0.00;
			     String sqlsk="select sum(round(op_qty*cost_price,2)) stkvalue from my_prddin where tr_no="+mastertr_no+" " ;
		           //System.out.println("stkvalue before--"+stkvalue+"-mastertr_no-"+mastertr_no);

			     ResultSet sfocss=stmtSTK.executeQuery(sqlsk);
				//System.out.println("++++"+sqlsk);
				while(sfocss.next())
				{ 
					stkvalue=sfocss.getDouble("stkvalue");
 
					 
				}
                //System.out.println("stkvalue==="+stkvalue+"stk1=="+stkvalue);

				if(reftype.equalsIgnoreCase("GRN"))
				{
					
				     String sqls1="select sum(op_qty*cost_price) stkvalue from my_prddin where invno="+docno+" " ;
					 //System.out.println("sqls1GRN===="+sqls1);
				     ResultSet sfocs1=stmtpuchase.executeQuery(sqls1);
	   ...
	   updtax=stval+descgridtax;
		...
		if(chkcshprchse==1){
		 sql1="insert into my_jvtran(date,ref_detail,doc_no,acno,description,curId,rate,dramount,ldramount,out_amount,id,trtype,ref_row,LAGE,cldocno,lbrrate,costtype,costcode,dTYPE,brhId,tr_no,STATUS)   "
		 		+ "values('"+masterdate+"','"+refdetails+"',"+docno+",'"+cashacno+"','"+descs+"','"+vendorcur+"','"+venrate+"',round("+dramountnw+",2),"+ldramount+",0,-1,3,0,0,0,'"+venrate+"',0,0,'PIV','"+session.getAttribute("BRANCHID").toString()+"',"+mastertr_no+",3)";
		}
		if(chkcshprchse==0){
			sql1="insert into my_jvtran(date,ref_detail,doc_no,acno,description,curId,rate,dramount,ldramount,out_amount,id,trtype,ref_row,LAGE,cldocno,lbrrate,costtype,costcode,dTYPE,brhId,tr_no,STATUS)   "
			 		+ "values('"+masterdate+"','"+refdetails+"',"+docno+",'"+venderaccount+"','"+descs+"','"+vendorcur+"','"+venrate+"',round("+dramountnw+",2),"+ldramount+",0,-1,3,0,0,0,'"+venrate+"',0,0,'PIV','"+session.getAttribute("BRANCHID").toString()+"',"+mastertr_no+",3)";
		}
		//System.out.println("--sql1editjvtran----"+sql1);
	     ...
	     double exprate=1;
			...
			for(int i2=0;i2< exparray.size();i2++){
		    	
			     String[] singleexparray=exparray.get(i2).split("::");
			     if(!(singleexparray[1].trim().equalsIgnoreCase("undefined")|| singleexparray[1].trim().equalsIgnoreCase("NaN")||singleexparray[1].trim().equalsIgnoreCase("")|| singleexparray[1].isEmpty()))
			     {
		   ...
		   ResultSet tass3 = stmt.executeQuery (sqls3);
			...
			// System.out.println("-----6--sqlveh----"+sqlveh) ;
				     ResultSet resultSet44 = stmt.executeQuery(sqlveh);
                      ...
                      //System.out.println("stkvalue=============="+stkvalue);
				      ...
				      else
				      {
				    	   ldramounts=pricetottal*rates ;
				    	   ldramounts=ClsCommon.Round(ldramounts, 2);
		   	           	//System.out.println("elseldramountsrate=============="+rates);
				      }
		    ...
		    if(clrs.next())
		    {
		     ...
		     if(vndcldocno>0)
		     {
				...
				if(rround>0 || rround<0)// less 0
				{
					...
					String sql13="insert into my_jvtran(date,ref_detail,doc_no,acno,description,curId,rate,dramount,ldramount,out_amount,id,trtype,ref_row,LAGE,cldocno,lbrrate,costtype,costcode,dTYPE,brhId,tr_no,STATUS)   "
							+ "values('"+masterdate+"','"+refdetails+"',"+docno+",'"+acnos3+"','"+descs+"','"+curris3+"','"+rates3+"',round("+rround+",2),round("+ldramounts3+",2)p,0,"+id1+",3,0,0,0,'"+rates3+"',0,0,'PIV','"+session.getAttribute("BRANCHID").toString()+"',"+mastertr_no+",3)";
...
int curate=0;
  ...
  //   System.out.println("LDR CHECK"+sql);
    ResultSet rsJv = ss.executeQuery(sql);
    while (rsJv.next()) {
        total=rsJv.getDouble("jvtotal");
...
//				System.out.println("---condn--"+condtn);
	  ...
	  return RESULTDATA;
		...
		try {
			...
			{	
	        	...
	        	String pySql=("select m.doc_no,m.voc_no,m.date,h.description,h.account,m.acno,round(aa.taxamount,3) netamount,m.description desc1 from "
	...
	public       ClspurchaseinvoiceBean getViewDetails(int masterdoc_no) throws SQLException {
		...
		String sqls="select "+sq1+" m.cashpurchase,m.billtype,m.totaltax,m.tax1,m.tax2,m.tax3,m.nettotaltax , m.exptotal,m.rdtype,if(m.rdtype='PO',m.rrefno,if(m.rdtype='GRN',m.rrefno,'')) rrefno,m.doc_no,m.voc_no,m.refno,m.date,h.description descs,h.account,m.acno,cr.doc_no as curid,cr.code as curr,round(cb.rate,2) as rate,m.amount,m.disstatus,m.disper,  " + 
				...
				+ "m.netamount-m.amount stval,(m.supplexp-(m.totaltax-(m.netamount-m.amount))) srvtotal,m.supplExp-(m.supplexp-(m.totaltax-(m.netamount-m.amount))) srvtaxamount    " + 
		...
		while (resultSet.next()) {
			...
			showBean.setSt(resultSet.getDouble("stval")); 
		...
		//duwn
			...
			showBean.setNettotal(resultSet.getDouble("srvtotal"));
			showBean.setOrderValue(resultSet.getDouble("nettotal"));
			showBean.setExpencenettotal(resultSet.getDouble("exptotal"));
			showBean.setDescgridhid(resultSet.getDouble("supplExp"));
            showBean.setDescgridtax(resultSet.getDouble("srvtaxamount"));
	...
	public   JSONArray reloadservs(String odocno) throws SQLException {
	 ...
	 //System.out.println("select srno,desc1 description,unitprice unitprice1,qty qty1,total total1,discount discount1,nettotal nettotal1 from my_ordser  where rdocno in ("+odocno+") ");
				ResultSet resultSet = stmtVeh1.executeQuery(pySql);
	...
	public   JSONArray maingridreload(String doc,String reftype,String reqdocno) throws SQLException {
		...
		try {
				...
				String pySql="select  din.batch_no,din.exp_date,bd.brandname,(sum(d.total)/sum(d.qty)) as cost_price,d.foc,d.stockid,at.mspecno specid,m1.rdtype,m1.rrefno,m.part_no productid,m.productname,  "
						...
						+ "   where m1.doc_no='"+doc+"' group by d.psrno ";
				...
				else if(reftype.equalsIgnoreCase("PO"))
				{
					...
					*/// orderdiscper,orderamount BATCHNO,bExpiry 
					...
					String pySql="select d.BATCHNO batch_no,bExpiry exp_date, bd.brandname,din.cost_price,aa.rowno refrowno,at.mspecno as specid,m1.rdtype,m1.rrefno,m.part_no productid, "
							...
							+ "  and aa.disper=d.disper "+joinsqls1+" where  m1.doc_no='"+doc+"'           group by d.stockid"+joinsqls+";";
  ...
  try {
    ...
    String  sql= "select a.catid,t.doc_no,t.account,t.description,t.curid,c.code currency,a.cldocno,c.type,round(cb.rate,2) rate,if(a.per_mob=0,a.per_tel,a.per_mob) mobile,a.chkrcm from my_head t inner join my_acbook a on t.cldocno=a.cldocno and "
...
/*				String resql=("select m.rdtype,if(m.rdtype='GRN',m.rrefno,'') rrefno,if(m.rdtype='DIR','Direct','Goods Receipt Note') type,m.doc_no,m.voc_no,m.refno,"
			...
			String resql="select coalesce(c.code,'') maincode,coalesce(acc.tinno,acc.trnnumber,'')cltinno,bh.tinno companytrno,format(round(sum(coalesce(d.total,0.0)),2),2) prdtotal,format(round(coalesce(s.srvtotal,0.0),2),2) srvtotal , "
					...
					+" m.disper, m.refinvno,DATE_FORMAT(m.refinvdate,'%d.%m.%Y') AS refinvdate, FORMAT(m.discount,2) discount,format(round(m.roundVal,2),2) roundVal,format(round(m.nettotal,2),2) netAmount,"
...
Statement stmtinvoice11 = conn.createStatement ();
			    ...
			    //	System.out.println("company---"+companysql);
				   ...
				   // System.out.println(">>>>>"+strSqldetail);  
					...
					/*for curid  to show corectly changed query by adding curid from docno*/
					String nettotQry="select format(round(m.supplexp,2),2) subtotal,format(round(m.nettotal,2),2) nettotal , c.code code  from my_srvm m left join my_curr c on c.doc_no=m.curid  where m.doc_no="+docno+";";
					...
					while(rsnettot.next()){
						...
						String totamtwrds = c.convertAmountToWords(rsnettot.getString("nettotal").replace(",", ""));
						String[] words = totamtwrds.split(" ");
				          // String totamtwrdsremove = String.join(" ", Arrays.copyOfRange(words, 1, words.length));   
				            ...
				            //removed code from amount to words and added curid from PIV docno currency 
							bean.setWordnetAmtPrint(rsnettot.getString("code")+ " " +totamtwrdsremove+"");
						//System.out.println("amounttowords------"+bean.getWordnetAmtPrint());
					}
